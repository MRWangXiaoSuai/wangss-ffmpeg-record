...
static int decode_bsfs_init(AVCodecContext *avctx)
{
    ...
    if (avci->bsf) //@wss add:已经配置了，直接返回
        return 0;

    ret = av_bsf_list_parse_str(codec->bsfs, &avci->bsf); //@wss add:分配filter并挂到avci->bsf->priv_class中
    if (ret < 0) {
    ...
    ret = avcodec_parameters_from_context(avci->bsf->par_in, avctx); //@wss add:用avctx初始化par_in参数，par_in用于存储要转换格式的编码数据
    if (ret < 0)
        goto fail;

    ret = av_bsf_init(avci->bsf); //@wss add:参数匹配判断，以及出入参拷贝，最后调用filter的init函数初始化
    if (ret < 0)
        goto fail;
...
int attribute_align_arg avcodec_send_packet(AVCodecContext *avctx, const AVPacket *avpkt)
{
    ...
    if (avpkt && (avpkt->data || avpkt->side_data_elems)) {
        ret = av_packet_ref(avci->buffer_pkt, avpkt); //@wss add:avpkt->copy-> buffer_pkt
        if (ret < 0)
            return ret;
    }

    ret = av_bsf_send_packet(avci->bsf, avci->buffer_pkt); //@wss add:发包给bsf解码
    if (ret < 0) {
        av_packet_unref(avci->buffer_pkt);
        return ret;
    }
...
{
    AVCodecInternal *avci = avctx->internal; //@wss add:AVCodecInterval存储的编解码信息相关上下文，在avcodec_open2()函数中只是做了分配内存
    int ret = 0;
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (avctx->codec->max_lowres < avctx->lowres || avctx->lowres < 0) { //@wss add:lowres低分辨率解码？
        av_log(avctx, AV_LOG_WARNING, "The maximum value for lowres supported by the decoder is %d\n",
               avctx->codec->max_lowres);
        avctx->lowres = avctx->codec->max_lowres;
    }
    if (avctx->sub_charenc) { //@wss add:标题字符编码
        if (avctx->codec_type != AVMEDIA_TYPE_SUBTITLE) {
            av_log(avctx, AV_LOG_ERROR, "Character encoding is only "
                   "supported with subtitles codecs\n");
            return AVERROR(EINVAL);
        } else if (avctx->codec_descriptor->props & AV_CODEC_PROP_BITMAP_SUB) { //@wss add:该标志位表示标题数据可以直接读取，因此啥都不做
    ...
    avctx->pts_correction_num_faulty_dts = 0; //@wss add:统计到的错误的pts和dts的数量
    avctx->pts_correction_last_pts =
    avctx->pts_correction_last_dts = INT64_MIN; //@wss add:最后一帧的pts和dts值

    if (   !CONFIG_GRAY && avctx->flags & AV_CODEC_FLAG_GRAY
        && avctx->codec_descriptor->type == AVMEDIA_TYPE_VIDEO) //@wss add:flags标志只解码灰度图，但是并没有配置
        av_log(avctx, AV_LOG_WARNING,
    ...
    ret = decode_bsfs_init(avctx); //@wss add:bsf bit stream filter,码流过滤器，其作用是将AVCC格式码流转换成Annex B格式(start code)的码流
    if (ret < 0)
        return ret;
