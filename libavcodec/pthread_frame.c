...
{
    PerThreadContext *p = arg; //@wss add:线程上下文，创建线程时带进来的参数
    AVCodecContext *avctx = p->avctx; //@wss add:codec上下文，已赋值
    const FFCodec *codec = ffcodec(avctx->codec);

    thread_set_name(p); //@wss add:给线程设置一个专属名称

    pthread_mutex_lock(&p->mutex);
    while (1) {
        while (atomic_load(&p->state) == STATE_INPUT_READY && !p->die)
            pthread_cond_wait(&p->input_cond, &p->mutex); //@wss add:线程启动之后在此休眠，等待packet submit，等待唤醒
...
#endif
            ) //@wss add:如果codec未实现update_thread_context()和线程安全的get_buffer()，则必须在解码完成后才能将状态转换为STATUS_SETUP_FINISHED，意味着下一个线程只能在当前线程解码完成后才能开始解码
            ff_thread_finish_setup(avctx); //@wss add:更改状态为STATUS_SETUP_FINISHED，广播唤醒其他线程
FF_ENABLE_DEPRECATION_WARNINGS
         ...
         * cannot be true here. */
        av_assert0(!p->hwaccel_serializing); //@wss add:断言硬件的状态

        /* if the previous thread uses hwaccel then we take the lock to ensure
         * the threads don't run concurrently */
        if (avctx->hwaccel) { //@wss add:如果使用了硬件加速，设置硬件的状态
        ...
        p->got_frame = 0;
        p->result = codec->cb.decode(avctx, p->frame, &p->got_frame, p->avpkt); //@wss add:解码

        if ((p->result < 0 || !p->got_frame) && p->frame->buf[0]) //@wss add:没解出数据
            ff_thread_release_buffer(avctx, p->frame);

        if (atomic_load(&p->state) == STATE_SETTING_UP) //@wss add:线程状态调整
            ff_thread_finish_setup(avctx);

        if (p->hwaccel_serializing) { //@wss add；不支持硬件解码，擦除硬件状态
        ...
        atomic_store(&p->state, STATE_INPUT_READY); //@wss add:更改状态

        pthread_cond_broadcast(&p->progress_cond); //@wss add:广播通知其他线程
...
FF_ENABLE_DEPRECATION_WARNINGS
    ...
    if (for_user) {
        if (codec->update_thread_context_for_user) //@wss add:创建线程前更新参数，如果不支持update_thread_context_for_user以及线程安全的get_buffer,则多线程背后仍是单线程操作
...
void ff_thread_finish_setup(AVCodecContext *avctx) {
     ...
     * state after decoding, without requiring synchronization @wss add:保存硬件加速状态留给其他线程使用*/
    ...
    atomic_store(&p->state, STATE_SETUP_FINISHED); //@wss add:把状态调整为STATE_SETUP_FINISHED

    pthread_cond_broadcast(&p->progress_cond); //@wss add:广播通知其他线程解码
    pthread_mutex_unlock(&p->progress_mutex);
}
                               ...
                               const FFCodec *codec, int first)
{
    ...
    atomic_init(&p->state, STATE_INPUT_READY); //@wss add:原子操作，初始化线程状态为STATE_INPUT_READY

    copy = av_memdup(avctx, sizeof(*avctx)); //@wss add:copy codec上下文到copy
    if (!copy)
        return AVERROR(ENOMEM);
    copy->priv_data = NULL; //@wss add:priv_data置NULL

    /* From now on, this PerThreadContext will be cleaned up by
     * ff_frame_thread_free in case of errors. */
    (*threads_to_free)++;
    //@wss add:这里每个单个线程都拥有一份父线程上下文和编解码上下文参数
    p->parent = fctx; //@wss add:父线程上下文
    p->avctx  = copy; //@wss add:编解码上下文

    copy->internal = av_mallocz(sizeof(*copy->internal));
    if (!copy->internal)
        return AVERROR(ENOMEM);
    copy->internal->thread_ctx = p; //@wss add:单个线程的上下文又让编解码上下文来管理

    copy->delay = avctx->delay;

    if (codec->priv_data_size) { //@wss add:copy priv_data数据
    ...
    err = ff_pthread_init(p, per_thread_offsets); //@wss add:初始化mutex和条件变量
    if (err < 0)
        return err;

    if (!(p->frame = av_frame_alloc()) ||
        !(p->avpkt = av_packet_alloc())) //@wss add:为输入和输出缓冲器分配内存
        return AVERROR(ENOMEM);
    copy->internal->last_pkt_props = p->avpkt;

    if (!first)
        copy->internal->is_copy = 1; //@wss add:只有第一个线程才会置0，用于控制extradata只解码一次

    if (codec->init) { //@wss add:codec->init函数指针有值
        err = codec->init(copy); //@wss add:codec初始化
        if (err < 0) {
    ...
    p->thread_init = NEEDS_CLOSE; //@wss add:初始化为NEEDS_CLOSE

    if (first)
        update_context_from_thread(avctx, copy, 1); //@wss add:用copy更新avctx，avctx会用于后续线程的上下文，保证后续线程使用的codec上下文和第一个线程保持一致，注意最后一个参数为1

    atomic_init(&p->debug_threads, (copy->debug & FF_DEBUG_THREADS) != 0);

    err = AVERROR(pthread_create(&p->thread, NULL, frame_worker_thread, p)); //@wss add:创建解码线程
    if (err < 0)
        return err;
    p->thread_init = INITIALIZED; //@wss add:解码线程创建完毕，更新状态为INITIALIZED (Thread has been properly set up)
...
int ff_frame_thread_init(AVCodecContext *avctx)
{
    ...
    if (!thread_count) { //@wss add:大部分情况下设置为1，如果没设置则根据CPU核心数来判断
        int nb_cpus = av_cpu_count(); //@wss add:获取CPU线程数
        // use number of cores + 1 as thread count if there is more than one
        if (nb_cpus > 1)
            thread_count = avctx->thread_count = FFMIN(nb_cpus + 1, MAX_AUTO_THREADS); //@wss add:解码线程数限制到16内
        else
            thread_count = avctx->thread_count = 1;
    }

    if (thread_count <= 1) { //@wss add:CPU只有一个线程核心，直接返回 =1就是单线程，没必要开启多线程
    ...
    err = ff_pthread_init(fctx, thread_ctx_offsets); //@wss add:FrameThreadContext线程上下文的mutex，条件变量初始化
    if (err < 0) {
        ff_pthread_free(fctx, thread_ctx_offsets);
        av_freep(&avctx->internal->thread_ctx);
        return err;
    ...
    fctx->async_lock = 1; //@wss add:同步锁置1
    fctx->delaying = 1; //@wss add:delaying初始化为1

    if (codec->p.type == AVMEDIA_TYPE_VIDEO)
        avctx->delay = avctx->thread_count - 1; //@wss add:delay的含义是解码输出相对编码输入的帧序号的差值(每个线程输出一帧)

    fctx->threads = av_calloc(thread_count, sizeof(*fctx->threads)); //@wss add:为每个线程的上下文分配内存
    if (!fctx->threads) {
        err = AVERROR(ENOMEM);
        goto error;
    }
    ...
    for (; i < thread_count; ) {
        ...
        err = init_thread(p, &i, fctx, avctx, codec, first); //@wss add:对每个线程，初始化线程函数
        if (err < 0)
            goto error;
    }
