...
static void worker_func(void *priv, int jobnr, int threadnr, int nb_jobs, int nb_threads)
{
    ...
    ret = c->func ? c->func(avctx, (char *)c->args + c->job_size * jobnr)
                  : c->func2(avctx, c->args, jobnr, threadnr); //@wss add:对应的真正的执行函数
    if (c->rets)
        c->rets[jobnr] = ret;
}
...
static int thread_execute(AVCodecContext *avctx, action_func* func, void *arg, int *ret, int job_count, int job_size)
{
    ...
    avpriv_slicethread_execute(c->thread, job_count, !!c->mainfunc  ); //@wss add:执行slice level线程
    return 0;
}
...
int ff_slice_thread_init(AVCodecContext *avctx)
{
    ...
    if (av_codec_is_encoder(avctx->codec) &&
        ...
        avctx->height > 2800)
        thread_count = avctx->thread_count = 1; //@wss add:编码器，且编码器id为MPEG1VIDEO且高超过2800 线程数设置为1

    if (!thread_count) { //@wss add:线程数量为0
        int nb_cpus = av_cpu_count(); //@wss add:获取CPU核心数
        if  (avctx->height)
            nb_cpus = FFMIN(nb_cpus, (avctx->height+15)/16);
        // use number of cores + 1 as thread count if there is more than one
        if (nb_cpus > 1)
            thread_count = avctx->thread_count = FFMIN(nb_cpus + 1, MAX_AUTO_THREADS); //@wss add:最大同时支持16个线程
        else
            thread_count = avctx->thread_count = 1;
    }

    if (thread_count <= 1) { //@wss add:如果是编码且满足上面条件，或者CPU核心数只有1，直接返回
        avctx->active_thread_type = 0;
        return 0;
    }

    avctx->internal->thread_ctx = c = av_mallocz(sizeof(*c)); //@wss add:为slice级别线程上下文分配内存
    mainfunc = ffcodec(avctx->codec)->caps_internal & FF_CODEC_CAP_SLICE_THREAD_HAS_MF ? &main_function : NULL; //@wss add:如果设置了FF_CODEC_CAP_SLICE_THREAD_HAS_MF
    if (!c || (thread_count = avpriv_slicethread_create(&c->thread, avctx, worker_func, mainfunc, thread_count)) <= 1) { //@wss add:创建slice级别的线程
        if (c)
            avpriv_slicethread_free(&c->thread);
        av_freep(&avctx->internal->thread_ctx); //@wss add:清理工作
    ...
    avctx->execute = thread_execute; //@wss add:注册多线程处理函数，实际会调用execute或者execute2函数执行真正的解码操作
