...
found:
    ...
    if (parser->parser_init) {
        ret = parser->parser_init(s); //@wss add:如h264_parser.c，设置H264DSPContext(s->priv_data->H264DSPContext)参数
        if (ret != 0)
            goto err_out;
    }
    s->key_frame            = -1;
    ...
    s->pts_dts_delta        = INT_MIN; //@wss add:pts和dts的差值
    s->format               = -1; //@wss add:像素格式，yuv420，rgb24等
