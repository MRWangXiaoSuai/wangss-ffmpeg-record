...
static void * attribute_align_arg worker(void *v){
    ...
    while (!atomic_load(&c->exit)) {
        ...
        ret = ff_encode_encode_cb(avctx, pkt, frame, &task->got_packet); //@wss add:执行编码操作并获取编码数据存储在pkt中
#if FF_API_THREAD_SAFE_CALLBACKS
...
av_cold int ff_frame_thread_encoder_init(AVCodecContext *avctx)
{
    ...
    if(   !(avctx->thread_type & FF_THREAD_FRAME)
       || !(avctx->codec->capabilities & AV_CODEC_CAP_FRAME_THREADS)) //@wss add:多线程编码(编码是否有多线程的含义)，从此处看，支支持帧模式
        return 0;
    ...
    if (avctx->codec_id == AV_CODEC_ID_HUFFYUV ||
        avctx->codec_id == AV_CODEC_ID_FFVHUFF) { //@wss add:这是什么编码类型
        int warn = 0;
        int64_t tmp;
    ...
    if(!avctx->thread_count) {
        avctx->thread_count = av_cpu_count(); //@wss add:获取CPU核心数量,线程数设置最大为64
    ...
    c->max_tasks = avctx->thread_count + 2; //@wss add:max_task为什么是线程数+2
    for (unsigned j = 0; j < c->max_tasks; j++) {
        if (!(c->tasks[j].indata  = av_frame_alloc()) ||
            !(c->tasks[j].outdata = av_packet_alloc())) { //@wss add:分配编码数据和原始数据的缓冲区
            ret = AVERROR(ENOMEM);
            goto fail;
        }
    }
    ...
    for(i=0; i<avctx->thread_count ; i++){
        void *tmpv;
        thread_avctx = avcodec_alloc_context3(avctx->codec); //@wss add:用codec参数赋值线程上下文，并对thread_avctx初始化
        if (!thread_avctx) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        ...
        *thread_avctx = *avctx;
        ...
        if ((ret = avcodec_open2(thread_avctx, avctx->codec, NULL)) < 0) //@wss add:多线程，根据thread_avctx打开每个线程的编解码器
            goto fail;
        av_assert0(!thread_avctx->internal->frame_thread_encoder);
        thread_avctx->internal->frame_thread_encoder = c;
        if ((ret = pthread_create(&c->worker[i], NULL, worker, thread_avctx))) { //@wss add:创建执行编码操作的线程
            ret = AVERROR(ret);
            goto fail;
        }
    }
