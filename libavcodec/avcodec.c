    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if (bits_per_sample) {
            ...
            if (bit_rate > INT64_MAX / bits_per_sample) {
                ...
                bit_rate *= bits_per_sample; //@wss add:音频码率 = 采样数 * 信道数 * 采样位数
        } else
            bit_rate = ctx->bit_rate;
        break;
    default:
...
int attribute_align_arg avcodec_open2(AVCodecContext *avctx, const AVCodec *codec, AVDictionary **options)
{
    ...
    if (!codec && !avctx->codec) { //@wss add:检查参数有效性
    ...
    if (avctx->extradata_size < 0 || avctx->extradata_size >= FF_MAX_EXTRADATA_SIZE) //@wss add:没有extradata，或者extradata_size太大导致存储的数据可能被截断，无法打开codec，返回错误
        return AVERROR(EINVAL);
    ...
    if (!(avctx->coded_width && avctx->coded_height && avctx->width && avctx->height &&
        ...
        if (avctx->coded_width && avctx->coded_height)
            ret = ff_set_dimensions(avctx, avctx->coded_width, avctx->coded_height); //@wss add:重新设置尺寸(宽高)
        else if (avctx->width && avctx->height)
            ret = ff_set_dimensions(avctx, avctx->width, avctx->height);
        if (ret < 0)
            goto free_and_end;
    }
    //@wss add:检查宽高和img中允许的最多像素数
    if ((avctx->coded_width || avctx->coded_height || avctx->width || avctx->height)
...
FF_DISABLE_DEPRECATION_WARNINGS
    /* compat wrapper for old-style callers */ //@wss add:音频信道布局以及数量检查，FF_API_OLD_CHANNEL_LAYOUT
...
FF_ENABLE_DEPRECATION_WARNINGS
     ...
     * in particular checks that nb_channels is set for all audio encoders. @wss add:检查音频编码器信道数*/
    ...
    if (avctx->ch_layout.nb_channels && !av_channel_layout_check(&avctx->ch_layout)) { //@wss add:检查音频通道布局
    ...
    avctx->codec_descriptor = avcodec_descriptor_get(avctx->codec_id); //@wss add:only decoder,获得解码器描述
    //@wss add:检查编码器是否处于实验阶段(意味着不可靠)，并且相关标志设置
    if ((avctx->codec->capabilities & AV_CODEC_CAP_EXPERIMENTAL) &&
        ...
        codec2 = av_codec_is_encoder(codec) ? avcodec_find_encoder(codec->id) : avcodec_find_decoder(codec->id); //@wss add:查找编解码器
        if (!(codec2->capabilities & AV_CODEC_CAP_EXPERIMENTAL))
            av_log(avctx, AV_LOG_ERROR, "Alternatively use the non experimental %s '%s'.\n",
                codec_string, codec2->name);
        ret = AVERROR_EXPERIMENTAL;
        goto free_and_end; //@wss add:不管找不找得到，走到这都不往下走了，直接释放资源
    }
    //@wss add:音频timebase就是采样率 检查音频时间基设置
    if (avctx->codec_type == AVMEDIA_TYPE_AUDIO &&
        (!avctx->time_base.num || !avctx->time_base.den)) {
        avctx->time_base.num = 1;
        avctx->time_base.den = avctx->sample_rate;
    }
    //@wss add:到此处，以上，都是检查AVCodecContext编码上下文参数
    //@wss add:检查参数没什么问题，下面就是对编解码器做一些处理了
    if (av_codec_is_encoder(avctx->codec))
        ret = ff_encode_preinit(avctx); //@wss add:音视频编码的参数判断，赋值以及多线程编码的开启
    else
        ret = ff_decode_preinit(avctx); //@wss add:音视频解码的参数判断，赋值以及码流过滤器的设置
    if (ret < 0)
        goto free_and_end;

    if (HAVE_THREADS && !avci->frame_thread_encoder) { //@wss add:多线程只针对解码可配置
        /* Frame-threaded decoders call FFCodec.init for their child contexts. */
        lock_avcodec(codec2); //@wss add:加锁
        ret = ff_thread_init(avctx); //@wss add:根据类型创建对应的多线程函数
    ...
    if (!HAVE_THREADS && !(codec2->caps_internal & FF_CODEC_CAP_AUTO_THREADS))
        avctx->thread_count = 1; //@wss add:默认线程数量为1
    ...
    if (av_codec_is_decoder(avctx->codec)) { //@wss add:解码器检查
        if (!avctx->bit_rate)
            avctx->bit_rate = get_bit_rate(avctx); //@wss add:获取码率 音频码率 = 采样数 * 信道数 * 采样位数；
