...
static void validate_thread_parameters(AVCodecContext *avctx)
{
    ...
    } else if (!(ffcodec(avctx->codec)->caps_internal & FF_CODEC_CAP_AUTO_THREADS)) { //@wss add:编码只能是FF_CODEC_CAP_AUTO_THREADS
...
{
    validate_thread_parameters(avctx); //@wss add:对线程参数进行设置，确认当前多线程的类型，是frame还是slice

    if (avctx->active_thread_type&FF_THREAD_SLICE) //@wss add:片级别多线程解码，送进去的是slice
        return ff_slice_thread_init(avctx);
    else if (avctx->active_thread_type&FF_THREAD_FRAME) //@wss add:帧级别多线程解码(送进去的是整个完整帧)
...
av_cold int ff_pthread_init(void *obj, const unsigned offsets[])
{
    ...
    unsigned cnt = 0;
    int err;
//@wss add:##用在define中，表示把前后数据连接起来 dst是FrameThreadContext在cur_offset偏移的位置 此处初始化FrameThreadContext的thread_ctx, pthread_init_cnt,buffer_mutex, hwaccel_mutex, async_mutex,async_cond成员
