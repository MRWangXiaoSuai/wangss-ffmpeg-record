...
int av_bsf_init(AVBSFContext *ctx)
{
    ...
    /* check that the codec is supported @wss add:确定输入流的codec是否在filter支持的序列内，如果不支持则初始化失败直接返回*/
    ...
    ctx->time_base_out = ctx->time_base_in; //@wss add:出入参，出入time_base的拷贝

    if (ff_bsf(ctx->filter)->init) { //@wss add:调用filter的init函数
...
int av_bsf_list_finalize(AVBSFList **lst, AVBSFContext **bsf)
{
    ...
    if ((*lst)->nb_bsfs == 1) { //@wss add:一个filter
        *bsf = (*lst)->bsfs[0];
        av_freep(&(*lst)->bsfs);
        (*lst)->nb_bsfs = 0;
        goto end;
    ...
    ret = av_bsf_alloc(&list_bsf.p, bsf); //@wss add:一个filter链表
    if (ret < 0)
        return ret;
...
int av_bsf_list_parse_str(const char *str, AVBSFContext **bsf_lst)
{
    ...
    if (!str)
        return av_bsf_get_null_filter(bsf_lst); //@wss add:分配一个空的filter
    ...
    do {
        ...
        if (ret < 0)
            goto end;
    } while (*str && *++str); //@wss add:解析,分隔符前后的每个filter名，通过find_by_name找到对应filter添加到list上

    ret = av_bsf_list_finalize(&lst, bsf_lst); //@wss add:判断最后的filter内部具体形式，是一个filter还是一个filter链表，并添加到AVBSFContext内作为出参返回
end:
    if (ret < 0)
...
#else
    return av_bsf_alloc(&list_bsf.p, bsf); //@wss add:根据filter分配一个context
#endif
}
