...
static int encode_make_refcounted(AVCodecContext *avctx, AVPacket *avpkt)
{
    ...
    avpkt->data = NULL;
    ret = ff_get_encode_buffer(avctx, avpkt, avpkt->size, 0); //@wss add:获得编码数据
    if (ret < 0)
        return ret;
                        ...
                        AVFrame *frame, int *got_packet)
{
    ...
    ret = codec->cb.encode(avctx, avpkt, frame, got_packet); //@wss add:编码操作
    emms_c();
    av_assert0(ret <= 0);

    if (!ret && *got_packet) {
        if (avpkt->data) {
            ret = encode_make_refcounted(avctx, avpkt); //@wss add:获取编码数据
            if (ret < 0)
                goto unref;
        ...
        if (!(avctx->codec_descriptor->props & AV_CODEC_PROP_REORDER) ||
            !(avctx->codec->capabilities & AV_CODEC_CAP_DELAY)) //@wss add:有无B帧，没B帧的话，dts = pts
            avpkt->dts = avpkt->pts;
    } else {
unref:
        av_packet_unref(avpkt);
...
static int encode_preinit_video(AVCodecContext *avctx)
{
    ...
    if (avctx->codec->pix_fmts) {
        for (i = 0; avctx->codec->pix_fmts[i] != AV_PIX_FMT_NONE; i++)
            if (avctx->pix_fmt == avctx->codec->pix_fmts[i])
                break;
        if (avctx->codec->pix_fmts[i] == AV_PIX_FMT_NONE) { //@wss add:没找到合适的pixformat直接返回
            char buf[128];
    ...
    if (    avctx->bits_per_raw_sample < 0
        || (avctx->bits_per_raw_sample > 8 && pixdesc->comp[0].depth <= 8)) { //@wss add:采样位数
        av_log(avctx, AV_LOG_WARNING, "Specified bit depth %d not possible with the specified pixel formats depth %d\n",
            avctx->bits_per_raw_sample, pixdesc->comp[0].depth);
        avctx->bits_per_raw_sample = pixdesc->comp[0].depth;
    }
    if (avctx->width <= 0 || avctx->height <= 0) { //@wss add编码宽高
        av_log(avctx, AV_LOG_ERROR, "dimensions not set\n");
        return AVERROR(EINVAL);
    }

    if (avctx->ticks_per_frame && avctx->time_base.num && //@wss add:ticks_per_frame场编码帧率(不用管)
    ...
    if (avctx->hw_frames_ctx) { //@wss add:硬编码的描述信息
...
static int encode_preinit_audio(AVCodecContext *avctx)
{
    ...
    if (avctx->codec->sample_fmts) {
        ...
        if (avctx->codec->sample_fmts[i] == AV_SAMPLE_FMT_NONE) { //@wss add:没找到合适的采样格式
            char buf[128];
    ...
    if (avctx->codec->supported_samplerates) { //@wss add:采样率判断
    ...
    if (avctx->codec->ch_layouts) { //@wss add:通道布局以及通道数判断
    ...
    if (!avctx->bits_per_raw_sample) //@wss add:采样位数
...
{
    AVCodecInternal *avci = avctx->internal; //@wss add:在avcodec_open2()中只分配buffer_frame,buffer_pkt的内存，未初始化
    int ret = 0;
    ...
    case AVMEDIA_TYPE_VIDEO: ret = encode_preinit_video(avctx); break; //@wss add:视频编码参数的判断和重新赋值
    case AVMEDIA_TYPE_AUDIO: ret = encode_preinit_audio(avctx); break; //@wss add:音频编码参数的判断和重新赋值
    }
    if (ret < 0)
        return ret;

    if (   (avctx->codec_type == AVMEDIA_TYPE_VIDEO || avctx->codec_type == AVMEDIA_TYPE_AUDIO)
        && avctx->bit_rate>0 && avctx->bit_rate<1000) { //@wss add:视频或音频的平均码率太低
        av_log(avctx, AV_LOG_WARNING, "Bitrate %"PRId64" is extremely low, maybe you mean %"PRId64"k\n", avctx->bit_rate, avctx->bit_rate);
    }

    if (!avctx->rc_initial_buffer_occupancy) //@wss add:解码开始前应该加载到rc buffer的字节数(rc buffer似乎是解码缓冲区)
        avctx->rc_initial_buffer_occupancy = avctx->rc_buffer_size * 3LL / 4;

    if (avctx->codec_descriptor->props & AV_CODEC_PROP_INTRA_ONLY) //@wss add:codec属性，这里是只有帧内压缩
        avctx->internal->intra_only_flag = AV_PKT_FLAG_KEY; //@wss add:帧内压缩，都是关键帧
    ...
    if ((avctx->flags & AV_CODEC_FLAG_RECON_FRAME)) { //@wss add:重建帧，是IDR还是对已编码帧重新编码？
    ...
    if (CONFIG_FRAME_THREAD_ENCODER) {
        ret = ff_frame_thread_encoder_init(avctx); //@wss add:多线程编码 默认情况下是开启的
        if (ret < 0)
            return ret;
    }
