...
const AVCodec *av_codec_iterate(void **opaque)
{
    ...
    //@wss add:执行一次av_codec_init_static
    ff_thread_once(&av_codec_static_init, av_codec_init_static); //thread 保证执行一次av_codec_init_static函数，初始化codec_list数据 codec_list保存注册编解码器
...
{
    return find_codec(id, av_codec_is_encoder); //@wss add:av_codec_is_encoder判断函数
}
