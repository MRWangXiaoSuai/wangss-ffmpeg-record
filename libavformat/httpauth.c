...
//@wss add:生成认证消息 认证方式有两种Basic认证以及Digest认证
                                   ...
                                   const char *path, const char *method)
{
    ...
    if (state->auth_type == HTTP_AUTH_BASIC) { //@wss add:最常见的认证方式。在Basic认证中，客户端发送请求时，会在请求头中包含一个"Authorization"字段，该字段包含了经过Base64编码的用户名和密码。服务器收到请求后，会解码该字段并验证用户名和密码是否正确。
        ...
        av_strlcat(ptr, "\r\n", len - (ptr - authstr));
        av_free(decoded_auth);
    } else if (state->auth_type == HTTP_AUTH_DIGEST) { //@wss add:Digest认证是一种更安全的认证方式。在Digest认证中，服务器会向客户端发送一个随机数（称为"nonce"），客户端根据该随机数和用户密码计算一个摘要，并将其发送给服务器。服务器收到摘要后，会验证其有效性。Digest认证相对于Basic认证而言，更难以被中间人攻击截获密码。
