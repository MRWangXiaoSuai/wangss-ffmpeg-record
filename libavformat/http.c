...
static int http_open_cnx_internal(URLContext *h, AVDictionary **options)
{
    ...
    av_url_split(proto, sizeof(proto), auth, sizeof(auth),
                 ...
                 path1, sizeof(path1), s->location); //@wss add:split location指代的url
    ff_url_join(hoststr, sizeof(hoststr), NULL, NULL, hostname, port, NULL);

    env_http_proxy = getenv_utf8("http_proxy");
    proxy_path = s->http_proxy ? s->http_proxy : env_http_proxy; //@wss add:http 代理

    env_no_proxy = getenv_utf8("no_proxy");
    use_proxy  = !ff_http_match_no_proxy(env_no_proxy, hostname) &&
                 proxy_path && av_strstart(proxy_path, "http://", NULL); //@wss add:判断是否使用代理
    freeenv_utf8(env_no_proxy);

    if (!strcmp(proto, "https")) { //@wss add:不是https协议
        lower_proto = "tls";
        use_proxy   = 0;
        if (port < 0)
            port = 443; //@wss add:没指定端口，默认端口443
    ...
    if (port < 0)
        port = 80; //@wss add:https协议默认端口号80
    ...
    if (!s->hd) {
        err = ffurl_open_whitelist(&s->hd, buf, AVIO_FLAG_READ_WRITE,
                                   ...
                                   h->protocol_whitelist, h->protocol_blacklist, h); //@wss add:打开底层协议http，https
    }
...
redo:
    ...
    cur_auth_type       = s->auth_state.auth_type; //@wss add:认证类型
...
static int http_write_reply(URLContext* h, int status_code)
{
    ...
    if (status_code < 0) //@wss add:控制块编码？
        body = 1;
    switch (status_code) {
    case AVERROR_HTTP_BAD_REQUEST:
    case 400:
    ...
    if (body) {
        ...
        message_len = snprintf(message, sizeof(message),
                 ...
                 s->headers ? s->headers : "",
                 reply_code,
                 reply_text); //@wss add:对请求报文生成响应报文
    } else {
        s->chunked_post = 1;
        message_len = snprintf(message, sizeof(message),
                 "HTTP/1.1 %03d %s\r\n"
                 ...
                 reply_text,
                 content_type,
                 s->headers ? s->headers : ""); //@wss add:块编码响应报文
    }
...
static int http_handshake(URLContext *c)
{
    ...
    case LOWER_PROTO:
        ...
        if ((ret = ffurl_handshake(cl)) > 0) //@wss add:握手
            return 2 + ret;
        if (ret < 0)
            return ret;
        ch->handshake_step = READ_HEADERS; //@wss add:更新握手状态
        ch->is_connected_server = 1;
        return 2;
    case READ_HEADERS:
        av_log(c, AV_LOG_TRACE, "Read headers\n");
        if ((err = http_read_header(c)) < 0) { //@wss add:提取http协议头信息
    ...
    case WRITE_REPLY_HEADERS:
        ...
        if ((err = http_write_reply(c, ch->reply_code)) < 0) //@wss add:根据提取的header信息生成响应报文 响应报文保存到c->hd(HTTPContext)
            return err;
...
static int http_listen(URLContext *h, const char *uri, int flags,
    ...
    ff_url_join(lower_url, sizeof(lower_url), lower_proto, NULL, hostname, port,
                NULL); //@wss add:http(tcp://ip:port) https(tls://ip:port)
    if ((ret = av_dict_set_int(options, "listen", s->listen, 0)) < 0) //@wss add:把listen标志设置进去
        goto fail;
    if ((ret = ffurl_open_whitelist(&s->hd, lower_url, AVIO_FLAG_READ_WRITE,
                                    &h->interrupt_callback, options,
                                    h->protocol_whitelist, h->protocol_blacklist, h
                                   )) < 0) //@wss add:调用tcp或者tls的url_open2()或者url_open()函数
        goto fail;
    s->handshake_step = LOWER_PROTO;
    if (s->listen == HTTP_SINGLE) { /* single client */
        s->reply_code = 200;
        while ((ret = http_handshake(h)) > 0); //@wss add:此函数做了三件事 1.执行握手操作；2.解析http协议头；3.生成响应报文；
    }
fail:
                     ...
                     AVDictionary **options)
{
    ...
    if( s->seekable == 1 ) //@wss add:能够seek，不是流
        h->is_streamed = 0;
    else
        h->is_streamed = 1;
    ...
    if (s->headers) {
        ...
        if (len < 2 || strcmp("\r\n", s->headers + len - 2)) { //@wss add:s->headers末尾没有结束符，手动添加
    ...
    if (s->listen) {
        return http_listen(h, uri, flags, options); //@wss add:服务器 打开底层协议(tcp/tls) 执行握手操作，解析http协议头，生成响应报文
    }
    ret = http_open_cnx(h, options); //@wss add:s->listen == 0走到这
bail_out:
    if (ret < 0) {
        av_dict_free(&s->chained_options);
        av_dict_free(&s->cookie_dict);
...
static int check_http_code(URLContext *h, int http_code, const char *end)
{
    ...
    if (http_code >= 400 && http_code < 600 &&
        ...
        end += strspn(end, SPACE_CHARS); //@wss add:定位到状态码描述
...
static int process_line(URLContext *h, char *line, int line_count)
{
    ...
    p = line;
    if (line_count == 0) {
        if (s->is_connected_server) { //@wss add:请求报文
            // HTTP method
            method = p;
            while (*p && !av_isspace(*p))
                p++;
            *(p++) = '\0';
            av_log(h, AV_LOG_TRACE, "Received method: %s\n", method); //@wss add:提取出请求方法 get post connect等
            if (s->method) {
                if (av_strcasecmp(s->method, method)) { //@wss add:解析出来的方法和设置的方法不匹配
                ...
                if (av_strcasecmp(auto_method, method)) { //@wss add:解析出来的方法和自动检测的方法不匹配
                    av_log(h, AV_LOG_ERROR, "Received and autodetected HTTP method did not match "
                           "(%s autodetected %s received)\n", auto_method, method);
                    return ff_http_averror(400, AVERROR(EIO));
                }
                if (!(s->method = av_strdup(method))) //@wss add:把解析出来的方法copy到s->method
                    return AVERROR(ENOMEM);
            }
            ...
            *(p++) = '\0';
            ...
            while (av_isspace(*p))
                p++;
            version = p;
            while (*p && !av_isspace(*p)) //@wss add:解析出来HTTP协议版本
                p++;
            *p = '\0';
            ...
            s->http_version = av_strndup(p, 3); //@wss add:解析HTTP协议版本
            while (!av_isspace(*p) && *p != '\0')
                p++;
            while (av_isspace(*p))
                p++;
            s->http_code = strtol(p, &end, 10); //@wss add:解析状态码

            av_log(h, AV_LOG_TRACE, "http_code=%d\n", s->http_code);

            if ((ret = check_http_code(h, s->http_code, end)) < 0) //@wss add:检查状态码，解析状态码描述
                return ret;
        }
    } else {
        while (*p != '\0' && *p != ':') //@wss add:除第一行，后续格式为 [头部字段名称:值\r\n]
            p++; //@wss add:此时p指向头部字段名称尾字符
        if (*p != ':')
            return 1;
        ...
        *p  = '\0';
        ...
        while (av_isspace(*p))
            p++;
        if (!av_strcasecmp(tag, "Location")) { //@wss add:响应头部字段名"Location":令客户端重定向至指定URL Location: http://www.usagidesign.jp/sample.html
            if ((ret = parse_location(s, p)) < 0)
                return ret;
        } else if (!av_strcasecmp(tag, "Content-Length") &&
                   s->filesize == UINT64_MAX) { //@wss add:实体首部字段名"Content-Length":实体主体的大小，单位字节 Content-Length: 15000
            s->filesize = strtoull(p, NULL, 10);
        } else if (!av_strcasecmp(tag, "Content-Range")) { //@wss add:实体首部字段名"Content-Range":实体主体的位置范围
            parse_content_range(h, p);
        } else if (!av_strcasecmp(tag, "Accept-Ranges") &&
                   !strncmp(p, "bytes", 5) &&
                   s->seekable == -1) { //@wss add:响应首部字段名"Accept-Ranges":是否接受字节范围请求 Accept-Ranges: bytes 当不能处理范围请求时，Accept-Ranges: none
            h->is_streamed = 0;
        } else if (!av_strcasecmp(tag, "Transfer-Encoding") &&
                   !av_strncasecmp(p, "chunked", 7)) { //@wss add:通用报文字段名"Transfer-Encoding":指定报文主体的传输编码方式 HTTP/1.1 的传输编码方式仅对分块传输编码有效。
            s->filesize  = UINT64_MAX;
            s->chunksize = 0;
        } else if (!av_strcasecmp(tag, "WWW-Authenticate")) { //@wss add:响应首部字段名"WWW-Authenticate":服务器对客户端的认证消息
            ff_http_auth_handle_header(&s->auth_state, tag, p);
        } else if (!av_strcasecmp(tag, "Authentication-Info")) { //@wss add:响应首部字段名"Authentication-Info":用于返回一些与授权会话相关的附加信息
            ff_http_auth_handle_header(&s->auth_state, tag, p);
        } else if (!av_strcasecmp(tag, "Proxy-Authenticate")) { //@wss add:响应首部字段名"Proxy-Authenticate":代理服务器对客户端的认证消息
            ff_http_auth_handle_header(&s->proxy_auth_state, tag, p);
        } else if (!av_strcasecmp(tag, "Connection")) { //@wss add:通用报文字段名"Connection":控制不再转发给代理的首部字段、管理持久连接。 Connection: close
            if (!strcmp(p, "close"))
                s->willclose = 1;
        } else if (!av_strcasecmp(tag, "Server")) { //@wss add:响应首部字段名"Server":告知客户端当前服务器上安装的HTTP服务器应用程序的信息。 Server: Apache/2.2.17 (Unix)
            if (!av_strcasecmp(p, "AkamaiGHost")) {
                s->is_akamai = 1;
            } else if (!av_strncasecmp(p, "MediaGateway", 12)) {
                s->is_mediagateway = 1;
            }
        } else if (!av_strcasecmp(tag, "Content-Type")) { //@wss add:实体首部字段名"Content-Type":实体主体内对象的媒体类型 Content-Type: text/html; charset=UTF-8
            av_free(s->mime_type);
            s->mime_type = av_strdup(p); //@wss add:更新了mime_type
        } else if (!av_strcasecmp(tag, "Set-Cookie")) { //@wss add:响应首部字段名"Set-Cookie":设置cookie路径 Set-Cookie:value [ ;expires=date][ ;domain=domain][ ;path=path][ ;secure]
            if (parse_cookie(s, p, &s->cookie_dict))
                av_log(h, AV_LOG_WARNING, "Unable to parse '%s'\n", p);
        } else if (!av_strcasecmp(tag, "Icy-MetaInt")) { //@wss add:响应首部字段名"Icy-MetaInt":设置两个元数据块之间的字节数 早期HTTP协议使用
            s->icy_metaint = strtoull(p, NULL, 10);
        } else if (!av_strncasecmp(tag, "Icy-", 4)) { //@wss add:响应/请求首部字段名"Icy-":设置流的一些参数
            if ((ret = parse_icy(s, tag, p)) < 0)
                return ret;
        } else if (!av_strcasecmp(tag, "Content-Encoding")) { //@wss add:实体首部字段名"Content-Encoding":告知客户端服务器对实体的主体部分选用的内容编码方式 Content-Encoding: gzip
            if ((ret = parse_content_encoding(h, p)) < 0)
                return ret;
        } else if (!av_strcasecmp(tag, "Expires")) { //@wss add:实体首部字段名"Expires":实体主体过期的日期时间
            parse_expires(s, p);
        } else if (!av_strcasecmp(tag, "Cache-Control")) { //@wss add:通用报文字段名"Cache-Control":控制缓存的行为 Cache-Control: private, max-age=0, no-cache
...
static int http_read_header(URLContext *h)
{
    ...
    for (;;) {
        ...
        err = process_line(h, line, s->line_count); //@wss add:line_count用来控制解析，一行行解析，等于0时解析请求行或者状态行
        if (err < 0)
            return err;
        if (err == 0) //@wss add:解析到尾部返回0，解析完毕，break
            break;
        s->line_count++;
    }

    // filesize from Content-Range can always be used, even if using chunked Transfer-Encoding
    if (s->filesize_from_content_range != UINT64_MAX) //@wss add:从"Context-Range"属性中解析，表示实体主体的位置范围
...
//@wss add:使用http代理会执行此函数 url:httpproxy://@authohost:port/host:port
static int http_proxy_open(URLContext *h, const char *uri, int flags)
{
    ...
    ff_url_join(hoststr, sizeof(hoststr), NULL, NULL, hostname, port, NULL); //@wss add:hoststr host:port
    path = pathbuf; //@wss add:pathbuf存储的字符串为 /host:port
    if (*path == '/')
        path++;

    ff_url_join(lower_url, sizeof(lower_url), "tcp", NULL, hostname, port,
                NULL); //@wss add:lower_url存储的为tcp://host:port
redo:
    ret = ffurl_open_whitelist(&s->hd, lower_url, AVIO_FLAG_READ_WRITE,
                               &h->interrupt_callback, NULL,
                               h->protocol_whitelist, h->protocol_blacklist, h); //@wss add:打开tcp协议，tcp链接过程
    if (ret < 0)
        return ret;

    authstr = ff_http_auth_create_response(&s->proxy_auth_state, auth,
                                           path, "CONNECT"); //@wss add:解析认证信息生成认证报文
    snprintf(s->buffer, sizeof(s->buffer),
             ...
             path,
             hoststr,
             authstr ? "Proxy-" : "", authstr ? authstr : ""); //@wss add:CONNECT HTTP一种方法，作用是把请求连接转换到透明的 TCP/IP 通道。此处生成的是CONNECT报文
    av_freep(&authstr);

    if ((ret = ffurl_write(s->hd, s->buffer, strlen(s->buffer))) < 0) //@wss add:把报文保存到URLContext中
        goto fail;

    s->buf_ptr    = s->buffer; //@wss add:更新buffer指针
    s->buf_end    = s->buffer;
    s->line_count = 0;
    s->filesize   = UINT64_MAX;
    cur_auth_type = s->proxy_auth_state.auth_type; //@wss add:更新当前认证类型Basic或者Digat
     ...
     */
    ret = http_read_header(h); //@wss add:解析http报头，并把相关属性设置到 h->priv_data(HTTPContext)中
    if (ret < 0)
        goto fail;
    ...
    if (s->http_code == 407 &&
        ...
        s->proxy_auth_state.auth_type != HTTP_AUTH_NONE && attempts < 2) { //@wss add:解析头部获取的状态码为407
        ffurl_closep(&s->hd);
        goto redo;
    }
