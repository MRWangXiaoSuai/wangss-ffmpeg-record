                           ...
                           const char *url, int flags, AVDictionary **options)
{
    ...
    return ffio_open_whitelist(pb, url, flags, &s->interrupt_callback, options, s->protocol_whitelist, s->protocol_blacklist); //@wss add:interrupt_callback 阻塞回调函数
}
...
AVFormatContext *avformat_alloc_context(void)
{
    ...
    s->io_open  = io_open_default; //@wss add:查找协议，初始化URLProtocol，打开URL，进一步初始化URLContext，根据URLContext初始化AVIOContext
    s->io_close = ff_format_io_close_default; //@wss add:清空数据和缓冲区
    s->io_close2= io_close2_default;

    av_opt_set_defaults(s); //@wss add:初始化 s->av_class->option的参数
