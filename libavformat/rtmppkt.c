                        ...
                        int chunk_size, RTMPPacket **prev_pkt, int *nb_prev_pkt)
{
    ...
    if (ffurl_read(h, &hdr, 1) != 1) //@Wss add:读取RTMP pkt header
        return AVERROR(EIO);
                                      ...
                                      int *nb_prev_pkt, uint8_t hdr)
{
    ...
    uint8_t buf[16]; //@wss add:保存basic hdr + message hdr + extend ts
    ...
    written++;
    channel_id = hdr & 0x3F; //@wss add:读取csid

    if (channel_id < 2) { //special case for channel number >= 64 @wss add:channel_id < 2,针对的场景是basic header占用2字节，channel id占用14bits情况
        buf[1] = 0;
        if (ffurl_read_complete(h, buf, channel_id + 1) != channel_id + 1)
            return AVERROR(EIO);
        written += channel_id + 1;
        channel_id = AV_RL16(buf) + 64; //@wss add:更新channel_id 存储的是与64差值，所以此处要加上64
    }
    ...
    hdr >>= 6; // header size indicator @wss add:获取包头长度
    if (hdr == RTMP_PS_ONEBYTE) { //@wss add:1byte, 是同一个RTMP pkt分包，ts相等
        ts_field = prev_pkt[channel_id].ts_field;
    } else {
        if (ffurl_read_complete(h, buf, 3) != 3) //@wss add:读取时间戳
            return AVERROR(EIO);
        written += 3;
        ts_field = AV_RB24(buf);
        if (hdr != RTMP_PS_FOURBYTES) { //@wss add:4bytes
            if (ffurl_read_complete(h, buf, 3) != 3) //@wss add:读取AMFSize
                return AVERROR(EIO);
            written += 3;
            size = AV_RB24(buf);
            if (ffurl_read_complete(h, buf, 1) != 1) //@wss add:读取AMFType
                return AVERROR(EIO);
            written++;
            type = buf[0];
            if (hdr == RTMP_PS_TWELVEBYTES) { //@wss add:12bytes
                if (ffurl_read_complete(h, buf, 4) != 4) //@wss add:读取streamID
                    return AVERROR(EIO);
                written += 4;
                extra = AV_RL32(buf);
            }
    ...
    if (ts_field == 0xFFFFFF) {
        if (ffurl_read_complete(h, buf, 4) != 4) //@wss add:读取exxtend ts
            return AVERROR(EIO);
        timestamp = AV_RB32(buf); //@wss add:位0xFFFFFF，时间戳存储在Externed timestamp，在message header后面占用4bytes
    } else {
        timestamp = ts_field;
    }
    if (hdr != RTMP_PS_TWELVEBYTES) //@wss add:只有12bytes存储存储的是完整的1时间戳，不然存储的都是时间增量
        timestamp += prev_pkt[channel_id].timestamp; //@wss add:对应客户端的use_delta 传输的是时间增量相加计算
    ...
    if (!prev_pkt[channel_id].read) { //@wss add:第一次解析
        if ((ret = ff_rtmp_packet_create(p, channel_id, type, timestamp,
                                         size)) < 0) //@wss add:为调用带进来的pkt分配内存
            return ret;
        p->read = written;
        p->offset = 0;
    ...
    if (ffurl_read_complete(h, p->data + p->offset, toread) != toread) { //@wss add:读取rtmp包体，保存在p中
    ...
    if (size > 0) { //@wss add:没读完
       RTMPPacket *prev = &prev_pkt[channel_id];
       prev->data = p->data; //@wss add:读取的数据保存在prev pkt中，最后一次读取在249行把数据保存在p中；
       prev->read = p->read;
       prev->offset = p->offset;
       p->data      = NULL;
       return AVERROR(EAGAIN); //@wss add:再来一次
    }

    prev_pkt[channel_id].read = 0; // read complete; reset if needed @wss add:读完了，结束
    return p->read;
}
                                 ...
                                 uint8_t hdr)
{
    while (1) {
        ...
        if (ret > 0 || ret != AVERROR(EAGAIN))
            return ret; //@wss add:读完了，直接返回读取的总字节数

        if (ffurl_read(h, &hdr, 1) != 1) //@wss add:还没读完，再读个hdr，重复
            return AVERROR(EIO);
    }
}
/*
    |--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|--------|
    |headtype|          timmer          |           AMFSize        |AMFType |               SteramID            |           extend timestamp        |

    |--------|
    |headtype| 又称为basic hdr(1-3bytes)前两位固定为chunk type(0-3),决定后面message hdr的长度，后续位数为csid，根据basic hdr的长度，csid取值范围有所变化
    完整的12bytes RTMP header:chunk type = 0
    8bytes只有headtype()，timmer delta，AMFSize，AMFType:chunk type = 1
    4bytes 只有 timmer delta:chunk type = 2
    1bytes message hdr长度为0:chunk type = 3
*/
                         ...
                         int *nb_prev_pkt)
{
    ...
    if ((ret = ff_rtmp_check_alloc_array(prev_pkt_ptr, nb_prev_pkt,
                                         pkt->channel_id)) < 0) //@wss add:写包保存历史包的缘故是因为这样做可以确定头部所需字节数，通过channel_id(表示basic hdr的csid)来区分
        return ret;
    prev_pkt = *prev_pkt_ptr;
    ...
    use_delta = prev_pkt[pkt->channel_id].channel_id &&
        ...
        pkt->timestamp >= prev_pkt[pkt->channel_id].timestamp; //@wss add:csid不为0，流id相等，时间戳增长 不需要使用12字节的full hdr

    timestamp = pkt->timestamp;
    if (use_delta) {
        timestamp -= prev_pkt[pkt->channel_id].timestamp; //@wss add:时间戳计算差值
    }
    if (timestamp >= 0xFFFFFF) {
        pkt->ts_field = 0xFFFFFF; //@wss add:time超过0xFFFFFF后，message hdr的time(3bytes)赋值为0xFFFFFF，time delta保存在后面的extend timestamp
    } else {
    ...
    if (use_delta) {
        if (pkt->type == prev_pkt[pkt->channel_id].type &&
            ...
            mode = RTMP_PS_FOURBYTES; //@wss add:当前RTMP pkt的AMFtype和AMFsize与上一个pkt一致 使用4bytes hdr
            if (pkt->ts_field == prev_pkt[pkt->channel_id].ts_field) //@wss add:时间也相同，说明是同一个RTMP pkt超过chucksize分包
                mode = RTMP_PS_ONEBYTE; //@wss add:使用1bytes hdr
        } else {
            mode = RTMP_PS_EIGHTBYTES; //@wss add:use_delta说明streamId同，size和type不同，使用8bytes hdr
        }
    }

    if (pkt->channel_id < 64) { //@wss add:basic hdr 1bytes
        bytestream_put_byte(&p, pkt->channel_id | (mode << 6)); //@wss add:封装header mode占basic hdr的前2bits
    } else if (pkt->channel_id < 64 + 256) { //@wss add:basic hdr 2bytes
        bytestream_put_byte(&p, 0               | (mode << 6));
        bytestream_put_byte(&p, pkt->channel_id - 64);
    } else { //@wss add:basic hdr 3bytes
        bytestream_put_byte(&p, 1               | (mode << 6));
        bytestream_put_le16(&p, pkt->channel_id - 64);
    }
    if (mode != RTMP_PS_ONEBYTE) {
        bytestream_put_be24(&p, pkt->ts_field); //@wss add:封装time 4bytes
        if (mode != RTMP_PS_FOURBYTES) {
            bytestream_put_be24(&p, pkt->size); //@wss add:封装AMFsize
            bytestream_put_byte(&p, pkt->type); //@wss add:封装AMFtype 8bytes
            if (mode == RTMP_PS_TWELVEBYTES) //@wss add:12字节
                bytestream_put_le32(&p, pkt->extra); //@wss add:封装streamid 12bytes
        }
    }
    if (pkt->ts_field == 0xFFFFFF)
        bytestream_put_be32(&p, timestamp); //@wss add:如果time为0xFFFFFF，封装extend time，否则extend time只占用0byte  到这里，RTMP hdr封装完毕
    // save history
    ...
    if ((ret = ffurl_write(h, pkt_hdr, p - pkt_hdr)) < 0) //@wss add:调用底层协议(tcp)写包 先写了个包头
        return ret;
    written = p - pkt_hdr + pkt->size; //@wss add:包体+包头长度
    while (off < pkt->size) {
        int towrite = FFMIN(chunk_size, pkt->size - off); //@wss add:每次写的size取chunk_size和剩余要写的size的最小值，保证每次写的数据不会超过chunk_size(128)
        if ((ret = ffurl_write(h, pkt->data + off, towrite)) < 0) //@wss add:写包体
            return ret;
        off += towrite;
        if (off < pkt->size) {
            uint8_t marker = 0xC0 | pkt->channel_id; //@wss add:获取RTMP包头长度
            if ((ret = ffurl_write(h, &marker, 1)) < 0) //@wss add:一个chunk_size没放完数据，分包，再写一次包头，只有1字节包头
                return ret;
            written++;
            if (pkt->ts_field == 0xFFFFFF) {
                uint8_t ts_header[4];
                AV_WB32(ts_header, timestamp);
                if ((ret = ffurl_write(h, ts_header, 4)) < 0) //@wss add:时间域为0xFFFFFF，extend timestamp也要加上
                    return ret;
                written += 4;
            }
        }
    } //@wss add:到此处，所有数据都分包且发送完成
    return written;
}
