...
int ff_rtmpe_gen_pub_key(URLContext *h, uint8_t *buf)
{
    ...
    //@wss add:key在前计算，768 = 4(time) + 4(ver) + 764(total len) - 4(offset) key的offset在最后四字节，总长度为764
    offset = ff_rtmp_calc_digest_pos(buf, 768, 632, 8); //@wss add:计算key密钥位置，计算方法为四字节offset相加取余，余数为764字节的整体长度- 128字节的Key秘钥-4字节的自身长度(632)
    if (offset < 0)
        return offset;

    /* generate a Diffie-Hellmann public key */
    if ((ret = ff_dh_generate_public_key(rt->dh)) < 0) //@wss add:生成DH密钥，对于C1来说，只需要是随机数即可
        return ret;

    /* write the public key into the handshake buffer */
    if ((ret = ff_dh_write_public_key(rt->dh, buf + offset, 128)) < 0) //@wss add:把dh密钥写到digest密文中
        return ret;
...
static int rtmpe_open(URLContext *h, const char *uri, int flags)
{
    ...
    if (rt->tunneling) { //@wss add:tunneling标志使用RTMPHTTP版本
        if (port < 0)
            port = 80;
        ...
        if (port < 0)
            port = 1935;
        ff_url_join(url, sizeof(url), "tcp", NULL, host, port, NULL); //@wss add:默认版本是tcp
    }

    /* open the tcp or ffrtmphttp connection */
    if ((ret = ffurl_open_whitelist(&rt->stream, url, AVIO_FLAG_READ_WRITE,
                                    &h->interrupt_callback, NULL,
                                    h->protocol_whitelist, h->protocol_blacklist, h)) < 0) { //@wss add:调用对应的底层协议
        rtmpe_close(h);
        return ret;
    }
