...
static int ff_tls_open(URLContext *h, const char *uri, int flags, AVDictionary **options)
{
    ...
    if (tls_init() == -1) { //@wss add:初始化全局数据结构
        ret = AVERROR(EIO);
        goto fail;
    }
    ...
    p->ctx = !c->listen ? tls_client() : tls_server(); //@wss add:创建tls链接对象
    if (!p->ctx) {
        ret = AVERROR(EIO);
        goto fail;
    }

    cfg = tls_config_new(); //@wss add:分配并初始化一个新的tls_config对象，在未来连接中使用
    if (!p->ctx) {
        ret = AVERROR(EIO);
        goto fail;
    }
    if (tls_config_set_protocols(cfg, TLS_PROTOCOLS_ALL) == -1) //@wss add:指定tls版本，TLS_PROTOCOLS_ALL代表所有支持的协议
        goto err_config;
    // While TLSv1.0 and TLSv1.1 are already enabled by the above,
    // we need to be less strict with ciphers so it works in practice.
    if (tls_config_set_ciphers(cfg, "compat") == -1) //@wss add:设置可以使用的密码列表
        goto err_config;
    if (c->ca_file && tls_config_set_ca_file(cfg, c->ca_file) == -1) //@wss add:Certificate Authority database file 设置证书颁发机构数据库文件
        goto err_config;
    if (c->cert_file && tls_config_set_cert_file(cfg, c->cert_file) == -1) //@wss add:Certificate file 设置证书文件
        goto err_config;
    if (c->key_file && tls_config_set_key_file(cfg, c->key_file) == -1) //@wss add:Private key file 设置私钥文件
        goto err_config;
    if (!c->verify) {
        tls_config_insecure_noverifycert(cfg); //@wss add:禁用证书验证和OSCP验证
        tls_config_insecure_noverifyname(cfg); //@wss add:禁用服务器名称验证
        tls_config_insecure_noverifytime(cfg); //@wss add:禁用证书的有效性检查和OSCP验证
    }
    if (tls_configure(p->ctx, cfg) == -1) //@wss add:配置tls链接对象
        goto err_ctx;
