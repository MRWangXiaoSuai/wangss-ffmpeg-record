...
static int rtmp_http_open(URLContext *h, const char *uri, int flags)
{
    ...
    av_url_split(NULL, 0, NULL, 0, rt->host, sizeof(rt->host), &rt->port,
                 NULL, 0, uri); //@wss add:就一个主机IP地址和端口号
     ...
     * in the URLs. */
    if (rt->tls) { //@wss add:tls协议是否启用
        if (rt->port < 0) //@wss add:没设置端口号，tls默认使用443 http + tls = https, tls是一种加密协议
            rt->port = RTMPTS_DEFAULT_PORT;
        ff_url_join(url, sizeof(url), "https", NULL, rt->host, rt->port, "/open/1"); //@wss add:https://ip:port/open/1
    } else {
        if (rt->port < 0)
            rt->port = RTMPT_DEFAULT_PORT; //@wss add:没设置端口号，默认使用80
        ff_url_join(url, sizeof(url), "http", NULL, rt->host, rt->port, "/open/1"); //@wss add:http://ip:port/open/1
    }

    /* alloc the http context */
    if ((ret = ffurl_alloc(&rt->stream, url, AVIO_FLAG_READ_WRITE, &h->interrupt_callback)) < 0) //@wss add:为RTMP_HTTPContext分配内存
        goto fail;
    ...
    av_opt_set(rt->stream->priv_data, "headers", headers, 0); //@wss add:设置属性，headers里面保存着Cache-Control,Context-type,User-Agent属性
    ...
    if ((ret = ffurl_connect(rt->stream, NULL)) < 0) //@wss add:调用https或者http协议的Url_open2()函数或者url_open()函数
        goto fail;
