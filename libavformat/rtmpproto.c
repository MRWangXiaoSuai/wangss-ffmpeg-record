...
static void del_tracked_method(RTMPContext *rt, int index)
{
    memmove(&rt->tracked_methods[index], &rt->tracked_methods[index + 1],
            sizeof(*rt->tracked_methods) * (rt->nb_tracked_methods - index - 1)); //@wss add:往前移
    rt->nb_tracked_methods--;
}
                               ...
                               char **tracked_method)
{
    ...
    for (i = 0; i < rt->nb_tracked_methods; i++) {
        ...
        *tracked_method = rt->tracked_methods[i].name;
        del_tracked_method(rt, i); //@wss add:删掉pkt_id对应的tracked_method项，后面的往前移
        break;
    }
...
static int rtmp_send_packet(RTMPContext *rt, RTMPPacket *pkt, int track)
{
    ...
    if (pkt->type == RTMP_PT_INVOKE && track) { //@wss add:客户端connect包走此
        GetByteContext gbc;
        char name[128];
        double pkt_id;
        int len;
    ...
    ret = ff_rtmp_packet_write(rt->stream, pkt, rt->out_chunk_size,
                               &rt->prev_pkt[1], &rt->nb_prev_pkt[1]); //@wss add:把RTMP包发给服务器 out_chunk_size默认128
fail:
 ...
 */
/*  @wss add:client connect Message
    +----------------------------------+---------------------+-----------------------------------------------------------------+
    ...
    +----------------------------------+---------------------+-----------------------------------------------------------------+
    | Command Object                   |	Object	         | 	        Command information obiect which has                   |
    ...
    +----------------------------------+----------------- ---+------------------------------------------------------------------+-----------------------------+
    |flashver                          |	String           |   Flash Player version. It isthe same string as returned         |		FMSC/1.0	          |
    |			                       |		     	     |    -by the Applicationscript getversion （） function            |			                   |
    +----------------------------------+---------------------+------------------------------------------------------------------+-----------------------------+
    |swfurl             	    	   |	String 	     	 | 		URL of the source SwF file                     	            |	file://c:/		          |
    |			                       |		     	     |		making the connection.                        	            |	F1vPlayer.swf	          |
    +----------------------------------+---------------------+------------------------------------------------------------------+-----------------------------+
    |tcurl	    		               |	String           |		URL of the Server.		   	                                |	rtmp://local	          |
    |			                       |		     	     |		It has the following format . 	   	                        |	host :1935/test	          |
    |			                       |		     	     |		protocol://servername:port/ I 	                            |	app/instance1	          |
    |			                       |		     	     |		appName / appInstance		                                |			                  |
    +----------------------------------+---------------------+------------------------------------------------------------------+-----------------------------+
    ...
    +----------------------------------+---------------------+------------------------------------------------------------------+-----------------------------+
    |pageurl			               |	String		     | 	   URL of the web page from 			                        |	    http://somehost/	  |
    |			                       |			         |	 where the SwF file was loaded.		                            |	        sample.html	      |
    +----------------------------------+---------------------+------------------------------------------------------------------+-----------------------------+
    |object Encoding		           |	Number		     |		AME encoding method.		                                |	            AMF3		  |
    +----------------------------------+---------------------+------------------------------------------------------------------+-----------------------------+
*/
...
static int gen_connect(URLContext *s, RTMPContext *rt)
{
    ...
    p = pkt.data; //@wss add:存储链接信息

    ff_amf_write_string(&p, "connect"); //@wss add:Command Name域，填充为connect字符串
    ff_amf_write_number(&p, ++rt->nb_invokes); //@wss add:Transaction ID域
    ff_amf_write_object_start(&p); //@wss add:Command Object域，填充为3
...
static int read_connect(URLContext *s, RTMPContext *rt)
{
    ...
    for (;;) {
        if ((ret = ff_rtmp_packet_read(rt->stream, &pkt, rt->in_chunk_size,
                                       &rt->prev_pkt[0], &rt->nb_prev_pkt[0])) < 0) //@wss add:读取并解析RTMP pkt
            return ret;
#ifdef DEBUG
        ff_rtmp_packet_dump(s, &pkt);
#endif
        if (pkt.type == RTMP_PT_CHUNK_SIZE) {
            if ((ret = handle_chunk_size(s, &pkt)) < 0) { //@wss add:更新chunk_size大小通知
                ff_rtmp_packet_destroy(&pkt);
                return ret;
            }
        } else if (pkt.type == RTMP_CTRL_ABORT_MESSAGE) { //@wss add:终止处理后续其他消息
            av_log(s, AV_LOG_ERROR, "received abort message\n");
            ff_rtmp_packet_destroy(&pkt);
            return AVERROR_UNKNOWN;
        } else if (pkt.type == RTMP_PT_BYTES_READ) { //@wss add:读取字节数
            av_log(s, AV_LOG_TRACE, "received acknowledgement\n");
        } else if (pkt.type == RTMP_PT_WINDOW_ACK_SIZE) { //@wss add:窗口确认通知
            if ((ret = handle_window_ack_size(s, &pkt)) < 0) {
                ff_rtmp_packet_destroy(&pkt);
                return ret;
            }
        } else if (pkt.type == RTMP_PT_SET_PEER_BW) { //@wss add:设置带宽通知
            if ((ret = handle_set_peer_bw(s, &pkt)) < 0) {
                ff_rtmp_packet_destroy(&pkt);
                return ret;
            }
        } else if (pkt.type == RTMP_PT_INVOKE) { //@wss add:命令消息
            // received RTMP Command Message
            break;
        } else {
            av_log(s, AV_LOG_ERROR, "Unknown control message type (%d)\n", pkt.type);
        }
        ff_rtmp_packet_destroy(&pkt); //@wss add:未知类型消息，destroy
    }
    //@wss add:RTMP_PT_BYTES_READ 以及 RTMP_PT_INVOKE走到此处
    cp = pkt.data;
    bytestream2_init(&gbc, cp, pkt.size);
    if (ff_amf_read_string(&gbc, command, sizeof(command), &stringlen)) { //@wss add:读取命令消息名称
        av_log(s, AV_LOG_ERROR, "Unable to read command string\n");
        ff_rtmp_packet_destroy(&pkt);
        return AVERROR_INVALIDDATA;
    }
    if (strcmp(command, "connect")) { //@wss add:如果不是connect
        av_log(s, AV_LOG_ERROR, "Expecting connect, got %s\n", command);
        ff_rtmp_packet_destroy(&pkt);
        return AVERROR_INVALIDDATA;
    }
    ret = ff_amf_read_number(&gbc, &seqnum); //@wss add:读取transcation id
    if (ret)
        av_log(s, AV_LOG_WARNING, "SeqNum not found\n");
    /* Here one could parse an AMF Object with data as flashVers and others. @wss add:解析rtmp pkt中携带的 name-value对*/
    ...
    if (!ret && strcmp(tmpstr, rt->app)) //@wss add:比较app即客户端要连接的服务器应用
    ...
    if ((ret = ff_rtmp_packet_create(&pkt, RTMP_NETWORK_CHANNEL,
                                     RTMP_PT_WINDOW_ACK_SIZE, 0, 4)) < 0) //@wss add:收到connect消息后，发送窗口确认通知
        return ret;
    p = pkt.data;
    ...
    bytestream_put_be32(&p, rt->max_sent_unacked); //@wss add:把最大未确认窗口大小通知给对端
    pkt.size = p - pkt.data;
    ret = ff_rtmp_packet_write(rt->stream, &pkt, rt->out_chunk_size,
                               &rt->prev_pkt[1], &rt->nb_prev_pkt[1]); //@wss add:发包
    ff_rtmp_packet_destroy(&pkt);
    if (ret < 0)
        return ret;
    // Set Peer Bandwidth
    if ((ret = ff_rtmp_packet_create(&pkt, RTMP_NETWORK_CHANNEL,
                                     RTMP_PT_SET_PEER_BW, 0, 5)) < 0) //@wss add:与对端确认带宽
        return ret;
    p = pkt.data;
    ...
    bytestream_put_byte(&p, 2); // dynamic 0:硬限制，立即更改窗口大小；1:软限制，可以更新也可以保留原值，但是原值一定小于更新的值；2:动态限制，如果上一次为硬限制，则此次也为硬限制，否则忽略；
    pkt.size = p - pkt.data;
    ret = ff_rtmp_packet_write(rt->stream, &pkt, rt->out_chunk_size,
                               &rt->prev_pkt[1], &rt->nb_prev_pkt[1]); //@wss add:发包
    ff_rtmp_packet_destroy(&pkt);
    ...
    // User control @wss add:发送用户控制消息给客户端，这一步正常需要在收到客户端的窗口确认消息之后才会执行
    if ((ret = ff_rtmp_packet_create(&pkt, RTMP_NETWORK_CHANNEL,
                                     RTMP_PT_USER_CONTROL, 0, 6)) < 0)
        return ret;

    p = pkt.data;
    bytestream_put_be16(&p, 0); // 0 -> Stream Begin @wss add:通知客户端流发信息开始传输
    ...
    ret = ff_rtmp_packet_write(rt->stream, &pkt, rt->out_chunk_size,
                               &rt->prev_pkt[1], &rt->nb_prev_pkt[1]); //@wss add:这里又确认了一次chunksize
    ff_rtmp_packet_destroy(&pkt);
    if (ret < 0)
        return ret;
    ...
    if ((ret = ff_rtmp_packet_create(&pkt, RTMP_SYSTEM_CHANNEL,
                                     RTMP_PT_INVOKE, 0,
                                     RTMP_PKTDATA_DEFAULT_SIZE)) < 0) //@wss add:通知客户端自己的连接状态
        return ret;

    p = pkt.data;
    ff_amf_write_string(&p, "_result"); //@wss add:command name
    ff_amf_write_number(&p, seqnum); //@wss add:transcation id
    ...
    if ((ret = ff_rtmp_packet_create(&pkt, RTMP_SYSTEM_CHANNEL,
                                     RTMP_PT_INVOKE, 0, 30)) < 0) //@wss add:发送服务控制消息
        return ret;
    p = pkt.data;
    ff_amf_write_string(&p, "onBWDone"); //@wss add:服务器发给客户端，关于之前客户端向服务器端请求的命令的响应结果
    ff_amf_write_number(&p, 0);
    ff_amf_write_null(&p);
    ff_amf_write_number(&p, 8192); //@wss add:注意，此处以上所有RTMP包都未封装RTMP header
    pkt.size = p - pkt.data;
    ret = ff_rtmp_packet_write(rt->stream, &pkt, rt->out_chunk_size,
                               &rt->prev_pkt[1], &rt->nb_prev_pkt[1]);
    ff_rtmp_packet_destroy(&pkt);
...
static int gen_publish(URLContext *s, RTMPContext *rt)
{
    ...
    ff_amf_write_null(&p);
    ff_amf_write_string(&p, rt->playpath);
    ff_amf_write_string(&p, "live"); //@wss add:live直播推流
...
static int gen_bytes_read(URLContext *s, RTMPContext *rt, uint32_t ts)
{
    ...
    if ((ret = ff_rtmp_packet_create(&pkt, RTMP_NETWORK_CHANNEL, RTMP_PT_BYTES_READ,
                                     ts, 4)) < 0) //@wss add:创建bytes_read命令包
        return ret;
                                  ...
                                  const char *subscribe)
{
    ...
    p = pkt.data;
    ff_amf_write_string(&p, "FCSubscribe"); //@wss add:似乎是用于兼容国外的Akamai,Edgecast,Limelight等CDN
    ff_amf_write_number(&p, ++rt->nb_invokes);
    ff_amf_write_null(&p);
...
static int rtmp_handshake_imprint_with_digest(uint8_t *buf, int encrypted)
{
    ...
    if (encrypted) //@wss add:加密模式，key在前digest在后，不加密key在后，digest在前 加密模式生成key密钥
        digest_pos = ff_rtmp_calc_digest_pos(buf, 772, 728, 776); //@wss add:排列为先key在digest 772 = 4(time) +4(version) + 764(key) 【776 = 12 + 764】, 728 = 764 - 4(offset) - 32(digest data)
    else
        digest_pos = ff_rtmp_calc_digest_pos(buf, 8, 728, 12); //@wss add:排列为先digest再key 8 = 4(time) +4(version) + 4(offset)[12] 不加密模式，key为乱码即可
    //@wss add:上面计算的digest_pos为digest data(32bytes)的索引位置
                                     ...
                                     uint8_t **out_data, int64_t *out_size)
{
    ...
    zs.avail_in = in_size;
    zs.next_in  = in_data;
    ret = inflateInit(&zs); //@wss add:ZLib压缩库解压初始化
    if (ret != Z_OK)
        return AVERROR_UNKNOWN;
    ...
    do {
        ...
        ret = inflate(&zs, Z_NO_FLUSH); //@wss add:解压操作
...
static int rtmp_calc_swfhash(URLContext *s)
{
    ...
    /* Get the SWF player file. @wss add:打开SWF文件*/
    ...
    if (!memcmp(in_data, "CWS", 3)) {
        ...
        *in_data = 'F'; // magic stuff
        ...
        if ((ret = rtmp_uncompress_swfplayer(in_data + 8, in_size - 8,
                                             &out_data, &out_size)) < 0) //@wss add:ZLIB解压
            goto fail;
        swfsize = out_size;
        swfdata = out_data;
#else
    ...
    /* Compute the SHA256 hash of the SWF player file. 计算SHA256(散列表)哈希值*/
    ...
    av_opt_set_bin(rt, "rtmp_swfhash", swfhash, 32, 0); //@wss add:hash值设置到选项中
    rt->swfsize = swfsize;
 ...
 * 握手过程客户端先发C0和C1包，服务器收到C0后发送S0,S1,S2，客户端收到S2后发送C2 握手完成
 * @return 0 if handshake succeeds, negative value otherwise
 */
static int rtmp_handshake(URLContext *s, RTMPContext *rt)
{
    AVLFG rnd;
    //@wss add:C0 1bytes 版本号(通常为3)  C1 1536bytes 4bytes(time) 4bytes(版本号) key(764bytes) digest(764bytes) 
    uint8_t tosend    [RTMP_HANDSHAKE_PACKET_SIZE+1] = {
        3,                // unencrypted data @wss add:简单握手，c0包，RTMP版本号3
        0, 0, 0, 0,       // client uptime @wss add:c1包，4bytes时间 4bytes版本号
        RTMP_CLIENT_VER1,
        RTMP_CLIENT_VER2,
        RTMP_CLIENT_VER3,
        RTMP_CLIENT_VER4,
    ...
    if (CONFIG_FFRTMPCRYPT_PROTOCOL && rt->encrypted) { //@wss add:加密协议生成DH密钥保存在key，不加密key为随机数即可
        ...
        tosend[0] = 6; //@wss add:密文，C0包中的version置6 明文为3
        tosend[5] = 128;
        tosend[6] = 0;
        tosend[7] = 3;
        tosend[8] = 2; //@wss add:版本号128.0.3.2

        /* Initialize the Diffie-Hellmann context and generate the public key
         * to send to the server. */
        if ((ret = ff_rtmpe_gen_pub_key(rt->stream, tosend + 1)) < 0) //@wss add:加密情况下生成DH密钥(key) 保存在rt->stream->priv_data->dh 并拷贝到tosend中(128bytes)
            return ret;
    }

    client_pos = rtmp_handshake_imprint_with_digest(tosend + 1, rt->encrypted); //@wss add:在tosend中嵌入digest data(32bytes)
    if (client_pos < 0)
        return client_pos;

    if ((ret = ffurl_write(rt->stream, tosend,
                           RTMP_HANDSHAKE_PACKET_SIZE + 1)) < 0) { //@wss add:调用协议url_write方法把数据嵌入到stream中 handsgake时没有RTMP包头，此处走的应该是tcp协议
        av_log(s, AV_LOG_ERROR, "Cannot write RTMP handshake request\n");
        return ret;
    }

    if ((ret = ffurl_read_complete(rt->stream, serverdata,
                                   RTMP_HANDSHAKE_PACKET_SIZE + 1)) < 0) { //@wss add:调用协议tcp_read方法把数据从stream中读到缓冲区 读取S0,S1(1537bytes)
        av_log(s, AV_LOG_ERROR, "Cannot read RTMP handshake response\n");
        return ret;
    }

    if ((ret = ffurl_read_complete(rt->stream, clientdata,
                                   RTMP_HANDSHAKE_PACKET_SIZE)) < 0) { //@wss add:读取s2(1536bytes)
        av_log(s, AV_LOG_ERROR, "Cannot read RTMP handshake response\n");
        return ret;
    }

    av_log(s, AV_LOG_DEBUG, "Type answer %d\n", serverdata[0]); //@wss add:S0 RTMP版本号 3
    av_log(s, AV_LOG_DEBUG, "Server version %d.%d.%d.%d\n", //@wss add: S1包含的版本号 S1包格式和C1一样
           serverdata[5], serverdata[6], serverdata[7], serverdata[8]);

    if (rt->is_input && serverdata[5] >= 3) { //@wss add:S1 1536bytes 4bytes(time) 4bytes(版本号) key(764bytes) digest(764bytes) 复杂握手，回复c2推流
        server_pos = rtmp_validate_digest(serverdata + 1, 772); //@wss add:验证接收到的服务器数据是否具有预期的digest值 返回digest位置索引
        if (server_pos < 0)
            return server_pos;

        if (!server_pos) {
            type = 1;
            server_pos = rtmp_validate_digest(serverdata + 1, 8); //@wss add:验证接收到的服务器数据是否具有预期的key值 
            if (server_pos < 0)
                return server_pos;
        ...
        //@wss add:到此处，客户端验证接受到S0,S1消息，生成C2消息
        ...
        if (CONFIG_FFRTMPCRYPT_PROTOCOL && rt->encrypted) {
            ...
            ff_rtmpe_encrypt_sig(rt->stream, tosend +
                                 ...
                                 serverdata[0]); //@wss add:加密
        }

        // write reply back to the server 写的是C2包么？
        if ((ret = ffurl_write(rt->stream, tosend,
        ...
        if ((ret = ffurl_write(rt->stream, serverdata + 1,
                               RTMP_HANDSHAKE_PACKET_SIZE)) < 0) //@wss add:发送c2包
            return ret;
                                  ...
                                  int size)
{
    ...
    *first_int  = AV_RB32(arraydata); //@wss add:time
    *second_int = AV_RB32(arraydata + 4); //@wss add:zero
    return 0;
}
                               ...
                               uint32_t second_int, char *arraydata, int size)
{
    ...
    AV_WB32(arraydata, first_int); //@wss add:C1携带的time
    AV_WB32(arraydata + 4, second_int); //@wss add:S1必须全为0
...
static int rtmp_server_handshake(URLContext *s, RTMPContext *rt)
{
    ...
    inoutsize = ffurl_read_complete(rt->stream, buffer, 1);       // Receive C0 读取一个字节数据到buffer S0包，版本号
    if (inoutsize <= 0) {
        av_log(s, AV_LOG_ERROR, "Unable to read handshake\n");
        return AVERROR(EIO);
    }
    // Check Version
    if (buffer[0] != 3) { //@wss add:复杂握手情况下version为6此处如何判断
    ...
    ret = rtmp_receive_hs_packet(rt, &hs_epoch, &zeroes, hs_c1,
                                 RTMP_HANDSHAKE_PACKET_SIZE); //@wss add:hs_epoch接收time，zeros接收0缓冲区
    if (ret) {
    ...
    for (randomidx = 8; randomidx < (RTMP_HANDSHAKE_PACKET_SIZE);
         randomidx += 4)
        AV_WB32(hs_s1 + randomidx, av_get_random_seed()); //@wss add:随机数填充跳过起始8bytes

    ret = rtmp_send_hs_packet(rt, hs_my_epoch, 0, hs_s1,
                              RTMP_HANDSHAKE_PACKET_SIZE); //@wss add:发送s1，time为C1携带的，0缓冲区全填0
    if (ret) {
    ...
    ret = rtmp_send_hs_packet(rt, hs_epoch, 0, hs_c1,
                              RTMP_HANDSHAKE_PACKET_SIZE); //@wss add:s2的time1填充C1携带的和time2填0，剩余的random就是C1携带的
    if (ret) {
    ...
    ret = rtmp_receive_hs_packet(rt, &temp, &zeroes, buffer,
                                 RTMP_HANDSHAKE_PACKET_SIZE); //@wss add:receive C2,temp存储time1，zeroes存储time2
    if (ret) {
    ...
    if (memcmp(buffer + 8, hs_s1 + 8,
               RTMP_HANDSHAKE_PACKET_SIZE - 8)) //@wss add:C2的random部分就是完全copy S1的random部分
        av_log(s, AV_LOG_WARNING,
...
static int handle_chunk_size(URLContext *s, RTMPPacket *pkt)
{
    ...
    if (!rt->is_input) { //@wss add:客户端走此，把chunk_size更改信息同步到服务器端
    ...
    rt->in_chunk_size = AV_RB32(pkt->data); //@wss add:服务器端接受到chunk_size更改通知，更新chunk_size 包体开始4字节保存chunk size
...
static int handle_user_control(URLContext *s, RTMPPacket *pkt)
{
    ...
    t = AV_RB16(pkt->data); //@wss add:包体开始2字节保存用户控制事件
    if (t == 6) { // PingRequest @wss add:ping命令
        if ((ret = gen_pong(s, rt, pkt)) < 0)
            return ret;
    } else if (t == 26) {
        if (rt->swfsize) {
            if ((ret = gen_swf_verification(s, rt)) < 0) //@wss add:swf确认消息
                return ret;
        } else {
...
static int handle_window_ack_size(URLContext *s, RTMPPacket *pkt)
{
    ...
    rt->receive_report_size >>= 1; //@wss add:x2
...
static int handle_connect_error(URLContext *s, const char *desc)
{
    ...
    if (!(cptr = strstr(desc, "authmod=adobe")) &&
        !(cptr = strstr(desc, "authmod=llnw"))) { //@wss add:authmod=adobe 和 authmod=llnw都不是desc的子串 adobe服务器和llnw服务器需要认证才能连接，其余服务器不需要认证，所以是未知错误
        av_log(s, AV_LOG_ERROR,
    ...
    while (*cptr && *cptr != ' ' && i < sizeof(authmod) - 1)
        authmod[i++] = *cptr++; //@wss add:把authmod的value保存在authmod数组中
    authmod[i] = '\0';

    if (!rt->username[0] || !rt->password[0]) { //@wss add:没有证书
        av_log(s, AV_LOG_ERROR, "No credentials set\n");
        return AVERROR_UNKNOWN;
    }

    if (strstr(desc, "?reason=authfailed")) { //@wss add:认证失败
        av_log(s, AV_LOG_ERROR, "Incorrect username/password\n");
        return AVERROR_UNKNOWN;
    } else if (strstr(desc, "?reason=nosuchuser")) { //@wss add:认证失败
        av_log(s, AV_LOG_ERROR, "Incorrect username\n");
        return AVERROR_UNKNOWN;
    }

    if (rt->auth_tried) { //@wss add:认证密码已发送失败，亦是认证失败
    ...
    if (strstr(desc, "code=403 need auth")) { //@wss add:返回信息是需要认证但是没带认证消息
        snprintf(rt->auth_params, sizeof(rt->auth_params),
                 "?authmod=%s&user=%s", authmod, rt->username); //@wss add:把认证信息写到认证参数里返回
        return 0;
    }

    if (!(cptr = strstr(desc, "?reason=needauth"))) { //@wss add:这里代表收到了认证信息且此用户存在，需要推流端发送认证密码
    ...
    if (!strcmp(authmod, "adobe")) { //@wss add:推流端收到服务器响应再把密码信息回馈给服务器，至此认证步骤才算完成
...
static int handle_invoke_error(URLContext *s, RTMPPacket *pkt)
{
    ...
    if ((ret = find_tracked_method(s, pkt, 9, &tracked_method)) < 0) //@wss add:这里是9 = string(2) + "error_"(7)
        return ret;

    if (!ff_amf_get_field_value(pkt->data + 9, data_end,
                                "description", tmpstr, sizeof(tmpstr))) { //@wss add:找到description字符串所在位置
        if (tracked_method && (!strcmp(tracked_method, "_checkbw")      ||
                               !strcmp(tracked_method, "releaseStream") ||
                               !strcmp(tracked_method, "FCSubscribe")   ||
                               !strcmp(tracked_method, "FCPublish"))) { //@wss add:检查错误方法是否是_checkbw,relaseSteam,FCSubScribe,FCPublish,不是任意一种走此处
            /* Gracefully ignore Adobe-specific historical artifact errors. */
            level = AV_LOG_WARNING;
            ret = 0;
        } else if (tracked_method && !strcmp(tracked_method, "getStreamLength")) { //@wss add:到此处上述所有方法返回error都不做处理
            level = rt->live ? AV_LOG_DEBUG : AV_LOG_WARNING;
            ret = 0;
        } else if (tracked_method && !strcmp(tracked_method, "connect")) { //@wss add:connect方法返回error，通常是因为没有证书缘故，这里加上证书重新connect
            ret = handle_connect_error(s, tmpstr); //@wss add:处理connect error 包括adobe和llnw服务器认证
            if (!ret) { //@wss add:返回0，重连
                rt->do_reconnect = 1;
...
static int write_begin(URLContext *s)
{
    ...
    if ((ret = ff_rtmp_packet_create(&spkt, RTMP_NETWORK_CHANNEL,
                                     RTMP_PT_USER_CONTROL, 0, 6)) < 0) { //@wss add:生成用户控制RTMP pkt
        av_log(s, AV_LOG_ERROR, "Unable to create response packet\n");
        return ret;
    }

    bytestream2_init_writer(&pbc, spkt.data, spkt.size);
    bytestream2_put_be16(&pbc, 0);          // 0 -> Stream Begin @wss add:服务器收到publish推流包，要给客户端回复stream begin信息
    bytestream2_put_be32(&pbc, rt->nb_streamid); //@wss add:封装streamid
                        ...
                        const char *status, const char *description, const char *details)
{
    ...
    pp = spkt.data;
    spkt.extra = pkt->extra; //@wss add:streamid
    ff_amf_write_string(&pp, "onStatus"); //@wss add:方法
    ff_amf_write_number(&pp, 0); //@wss add:id
    ff_amf_write_null(&pp); //@wss add:NULL

    ff_amf_write_object_start(&pp);
    ff_amf_write_field_name(&pp, "level"); //@wss add:key:level
    ff_amf_write_string(&pp, "status"); //@wss add:value:status
    ff_amf_write_field_name(&pp, "code"); //@wss add:key:code
    ff_amf_write_string(&pp, status); //@wss add:value:NetStream.Publish.Start 其中一个例子
    ...
    ret = ff_rtmp_packet_write(rt->stream, &spkt, rt->out_chunk_size,
                               &rt->prev_pkt[1], &rt->nb_prev_pkt[1]); //@wss add:send
    ff_rtmp_packet_destroy(&spkt);
...
static int send_invoke_response(URLContext *s, RTMPPacket *pkt)
{
    ...
    bytestream2_init(&gbc, p, pkt->size); //@wss add:gbc持有p的数据指针
    ...
    ret = ff_amf_read_number(&gbc, &seqnum); //@wss add:读取序列号作为唯一标识
    if (ret)
        return ret;
    ret = ff_amf_read_null(&gbc); //@wss add:读取NULL字符串 命令pkt body固有格式
    if (ret)
        return ret;
    if (!strcmp(command, "FCPublish") ||
        !strcmp(command, "publish")) { //@wss add:推流publish命令 FCPublish是服务器收到准备接收流通知，服务器只响应不做任何操作，publish是告诉客户端可以推流了，服务器响应stream begin此后正式开始推流
        ret = ff_amf_read_string(&gbc, filename,
                                 sizeof(filename), &stringlen); //@wss add:读取推流地址
        if (ret) {
        ...
        rt->state = STATE_RECEIVING; //@wss add:更改状态
    }

    if (!strcmp(command, "FCPublish")) { //@wss add:如果是FCPublish
        if ((ret = ff_rtmp_packet_create(&spkt, RTMP_SYSTEM_CHANNEL,
                                         RTMP_PT_INVOKE, 0,
                                         RTMP_PKTDATA_DEFAULT_SIZE)) < 0) {
            av_log(s, AV_LOG_ERROR, "Unable to create response packet\n");
        ...
        pp = spkt.data;
        ff_amf_write_string(&pp, "onFCPublish"); //@wss add:创建一个RTMP回复包，填充onFCPublish
    } else if (!strcmp(command, "publish")) { //@wss add:如果是Publish
        char statusmsg[128];
        snprintf(statusmsg, sizeof(statusmsg), "%s is now published", filename);
        ret = write_begin(s); //@wss add:给客户端回复stream begin信息
        if (ret < 0)
            return ret;

        // Send onStatus(NetStream.Publish.Start)
        return write_status(s, pkt, "NetStream.Publish.Start",
                            statusmsg, filename); //@wss add:给客户端回复状态更改信息
    } else if (!strcmp(command, "play")) { //@wss add:如果是Play 拉流播放信息 正常流程应该是服务端先setchunksize，然后回复streamIsRecord，然后是streambegin
        ret = write_begin(s); //@wss add:给客户端回复stream begin信息
        if (ret < 0)
            return ret;
        rt->state = STATE_SENDING; //@wss add:更改状态
        return write_status(s, pkt, "NetStream.Play.Start",
                            "playing stream", NULL); //@wss add:给客户端回复状态更改信息
    } else { //@wss add:other,回复result信息
        if ((ret = ff_rtmp_packet_create(&spkt, RTMP_SYSTEM_CHANNEL,
                                         RTMP_PT_INVOKE, 0,
                                         RTMP_PKTDATA_DEFAULT_SIZE)) < 0) {
            av_log(s, AV_LOG_ERROR, "Unable to create response packet\n");
        ...
        if (!strcmp(command, "createStream")) { //@wss add:如果是createstream生成信道
            rt->nb_streamid++;
            if (rt->nb_streamid == 0 || rt->nb_streamid == 2)
                rt->nb_streamid++; /* Values 0 and 2 are reserved */
            ff_amf_write_number(&pp, rt->nb_streamid); //@wss add:分配一个streamid 没考虑id超出可分配范围场景
    ...
    spkt.size = pp - spkt.data;
    ret = ff_rtmp_packet_write(rt->stream, &spkt, rt->out_chunk_size,
                               &rt->prev_pkt[1], &rt->nb_prev_pkt[1]); //@wss add:onFcPublish and other at here send
    ff_rtmp_packet_destroy(&spkt); //@wss add:没有考虑release stream的情况
    return ret;
}
...
static int read_number_result(RTMPPacket *pkt, double *number)
{
    ...
    if (ff_amf_read_number(&gbc, &numbuffer)) //@wss add:transcation ID
        return AVERROR_INVALIDDATA;
    // Value 3/4: Null
    if (ff_amf_read_null(&gbc))
        return AVERROR_INVALIDDATA;
    // Value 4/4: The response as AMF_NUMBER
    if (ff_amf_read_number(&gbc, &numbuffer)) //@wss add:stream ID
        return AVERROR_INVALIDDATA;
    else
        *number = numbuffer;
...
static int handle_invoke_result(URLContext *s, RTMPPacket *pkt)
{
    ...
    if ((ret = find_tracked_method(s, pkt, 10, &tracked_method)) < 0) //@wss add:10 = string(2) + "_result\0"(8)
        return ret;
    ...
    if (!strcmp(tracked_method, "connect")) { //@wss add:客户端收到 connect result
        if (!rt->is_input) { //@wss add:推流
            if ((ret = gen_release_stream(s, rt)) < 0) //@wss add:C->S 通知服务器释放一些媒体信道
                goto fail;

            if ((ret = gen_fcpublish_stream(s, rt)) < 0) //@wss add:C->S 通知服务器准备接收媒体流
                goto fail;
        } else {
            if ((ret = gen_window_ack_size(s, rt)) < 0) //@wss add:C->S 发送窗口确认消息
                goto fail;
        }

        if ((ret = gen_create_stream(s, rt)) < 0) //@wss add:C->S 发送"create stream"命令消息，通知服务器创建一个逻辑通道用于发送音视频，metadata等数据
            goto fail;
    ...
    } else if (!strcmp(tracked_method, "createStream")) { //@wss add:客户端收到 createStream创建流结果响应，此时流创建成功
        double stream_id;
        if (read_number_result(pkt, &stream_id)) { //@wss add:获取服务器相应的streamID，该ID用于唯一标识通道
            av_log(s, AV_LOG_WARNING, "Unexpected reply on connect()\n");
        } else {
            rt->stream_id = stream_id; //@wss add:保存ID
        }

        if (!rt->is_input) { //@wss add:C->S 推流，发送publish命令
            if ((ret = gen_publish(s, rt)) < 0)
                goto fail;
        } else { //@wss add:拉流
            if (rt->live != -1) {
                if ((ret = gen_get_stream_length(s, rt)) < 0) //@wss add:C->S 生成命令获取服务器流的时长
                    goto fail;
            }
            if ((ret = gen_play(s, rt)) < 0) //@wss add:C->S 生成play命令
                goto fail;
            if ((ret = gen_buffer_time(s, rt)) < 0) //@wss add:C->S 通知服务器客户端当前缓冲区时长 对应play2命令吗
                goto fail;
        }
    } else if (!strcmp(tracked_method, "getStreamLength")) { //@wss add:C->S 拉流获取当前流时长响应
        if (read_number_result(pkt, &rt->duration)) { //@wss add:读取流的时长
...
static int handle_invoke_status(URLContext *s, RTMPPacket *pkt)
{
    ...
    t = ff_amf_get_field_value(ptr, data_end, "level", tmpstr, sizeof(tmpstr)); //@wss add:查找"level"字符串
    if (!t && !strcmp(tmpstr, "error")) { //@wss add:level为error
    ...
    if (!t && !strcmp(tmpstr, "NetStream.Seek.Notify")) rt->state = STATE_PLAYING; //@wss add:根据反馈的code消息更新state
...
static int handle_invoke(URLContext *s, RTMPPacket *pkt)
{
    ...
    if (ff_amf_match_string(pkt->data, pkt->size, "_error")) { //@wss add:S->C 错误响应消息
        if ((ret = handle_invoke_error(s, pkt)) < 0)
            return ret;
    } else if (ff_amf_match_string(pkt->data, pkt->size, "_result")) { //@wss add:S->C 结果通知
        if ((ret = handle_invoke_result(s, pkt)) < 0)
            return ret;
    } else if (ff_amf_match_string(pkt->data, pkt->size, "onStatus")) { //@wss add:S->C 状态通知
        if ((ret = handle_invoke_status(s, pkt)) < 0)
            return ret;
    } else if (ff_amf_match_string(pkt->data, pkt->size, "onBWDone")) { //@wss add:S->C 检查带宽
        ...
        if ((ret = send_invoke_response(s, pkt)) < 0) //@wss add:C->S 服务器接受到一些命令对其做出响应
            return ret;
    }
...
static int update_offset(RTMPContext *rt, int size)
{
    ...
    if (rt->flv_off < rt->flv_size) { //@wss add:之前接受到数据
...
//@wss add:接收到audio,video data处理函数
static int append_flv_data(RTMPContext *rt, RTMPPacket *pkt, int skip)
{
    ...
    if (pkt->type == RTMP_PT_AUDIO) {
        rt->has_audio = 1; //@wss add:标志位置为1
    } else if (pkt->type == RTMP_PT_VIDEO) {
        rt->has_video = 1;
    }
    //@wss add:rtmp音视频数据以类似flv tag封包形式传输，15 = PreviousTagSize(4bytes) + Tag hdr(11bytes) 紧跟着的就是数据，video data = video tag hdr(5bytes) + data;audio data = audio tag hdr(2bytes) + data
    old_flv_size = update_offset(rt, size + 15);  //@wss add:这里确定flv_size为size(data) + 15

    if ((ret = av_reallocp(&rt->flv_data, rt->flv_size)) < 0) { //@wss add:分配data + flv tag 内存
        rt->flv_size = rt->flv_off = 0;
        return ret;
    }
    bytestream2_init_writer(&pbc, rt->flv_data, rt->flv_size); //@wss add:reset 指针
    bytestream2_skip_p(&pbc, old_flv_size); //@wss add:跳过PreviousTagSize(n)
    bytestream2_put_byte(&pbc, pkt->type); //@wss add:填充type
    bytestream2_put_be24(&pbc, size); //@wss add:填充size
    bytestream2_put_be24(&pbc, ts); //@wss add:填充ts
    bytestream2_put_byte(&pbc, ts >> 24); //@wss add:填充扩展时间戳
    bytestream2_put_be24(&pbc, 0); //@wss add:填充streamid
    bytestream2_put_buffer(&pbc, data, size); //@wss add:填充data 到此处应该内存分配完毕了
    bytestream2_put_be32(&pbc, size + RTMP_HEADER); //@wss add:填充PreviousTagSize(n+1)
...
static int handle_notify(URLContext *s, RTMPPacket *pkt)
{
    ...
    if (!strcmp(commandbuffer, "onMetaData")) { //@wss add:onMetaData通知
        ...
        if (bytestream2_get_byte(&gbc) == AMF_DATA_TYPE_MIXEDARRAY) { //@wss add:onMeataData字符串后面跟着的是8
            ...
            while (bytestream2_get_bytes_left(&gbc) > 3) { //@wss add:剩余字节数大于3
                if (ff_amf_get_string(&gbc, statusmsg, sizeof(statusmsg),
                                      &stringlen)) //@wss add:获取meessage信息
                    return AVERROR_INVALIDDATA;
                ...
                if (!strcmp(statusmsg, "videocodecid")) { //@wss add；主要从中解析出codecid信息来确定当前数据类型
                    rt->has_video = 1;
                }
    ...
    return append_flv_data(rt, pkt, skip); //@wss add:跳过skip，把后面数据append flvdata(rt->flvdata)
}
...
static int rtmp_parse_result(URLContext *s, RTMPContext *rt, RTMPPacket *pkt)
{
    ...
    switch (pkt->type) { //@wss add:pkt->data只保存rtmp包体部分，包头解析已经完成保存在其他属性
    ...
    case RTMP_PT_USER_CONTROL:
        if ((ret = handle_user_control(s, pkt)) < 0) //@wss add:处理用户控制，包括ping和swf确认
            return ret;
        break;
    ...
    case RTMP_PT_WINDOW_ACK_SIZE:
        ...
        break;
    case RTMP_PT_INVOKE:
        if ((ret = handle_invoke(s, pkt)) < 0) //@wss add:处理命令消息
            return ret;
        break;
    case RTMP_PT_VIDEO:
    case RTMP_PT_AUDIO:
...
static int get_packet(URLContext *s, int for_header)
{
    ...
    if (rt->state == STATE_STOPPED)
        return AVERROR_EOF; //@wss add:此返回值并不会结束get_packet循环调用

    for (;;) {
        RTMPPacket rpkt = { 0 };
        if ((ret = ff_rtmp_packet_read(rt->stream, &rpkt,
                                       rt->in_chunk_size, &rt->prev_pkt[0],
                                       &rt->nb_prev_pkt[0])) <= 0) { //@wss add:返回的是读取的数据总字节数 > 0 rpkt的内存分配在函数内部做
            if (ret == 0) {
                return AVERROR(EAGAIN); //@wss add:此返回值真正结束get_packet循环调用
            } else {
                return AVERROR(EIO);
            }
        }
        ...
        rt->last_timestamp = rpkt.timestamp; //@wss add:更新时间戳

        rt->bytes_read += ret; //@wss add:更新读取字节数
        if (rt->bytes_read - rt->last_bytes_read > rt->receive_report_size) { //@wss add:读取超过了要report给对端的size
            av_log(s, AV_LOG_DEBUG, "Sending bytes read report\n");
            if ((ret = gen_bytes_read(s, rt, rpkt.timestamp + 1)) < 0) { //@wss add:生成bytes_read命令包并发送
                ff_rtmp_packet_destroy(&rpkt);
                return ret;
            }
            rt->last_bytes_read = rt->bytes_read; //@wss add:更新字节数
        }


        ret = rtmp_parse_result(s, rt, &rpkt); //@wss add:解析RTMP pkt，对一些消息做出响应，metedata，notify，video/audio data在后面处理
        ...
        if (rt->state == STATE_SEEKING) { //@wss add:拖动播放时间轴操作此时还不允许
            ff_rtmp_packet_destroy(&rpkt);
        ...
        if (rt->do_reconnect && for_header) { //@wss add:do_reconnect场景为adobe和llnw服务器需要认证没有带认证消息
            ff_rtmp_packet_destroy(&rpkt);
            return 0;
        }
        if (rt->state == STATE_STOPPED) {
        ...
        if (for_header && (rt->state == STATE_PLAYING    ||
                           ...
                           rt->state == STATE_RECEIVING)) { //@wss add:此时状态还不可能变为上述这些
            ff_rtmp_packet_destroy(&rpkt);
            return 0;
        }
        if (!rpkt.size || !rt->is_input) { //@wss add:is_input为NULL continue，is_input似乎是个输入标志，如果为0，代表不接受输入，则不会处理下面的音视频数据
            ff_rtmp_packet_destroy(&rpkt);
            continue;
        }
        if (rpkt.type == RTMP_PT_VIDEO || rpkt.type == RTMP_PT_AUDIO) { //@wss add:处理音视频数据
            ret = append_flv_data(rt, &rpkt, 0);
            ff_rtmp_packet_destroy(&rpkt);
            return ret;
        } else if (rpkt.type == RTMP_PT_NOTIFY) { //@wss add:处理通知
            ret = handle_notify(s, &rpkt);
            ff_rtmp_packet_destroy(&rpkt);
            return ret;
        } else if (rpkt.type == RTMP_PT_METADATA) { //@wss add:处理metadata
...
static int rtmp_open(URLContext *s, const char *uri, int flags, AVDictionary **opts)
{
    ...
    //@wss add:rtmp://58.200.131.2:1935/livetv/cctv1  proto{rtmp}  path{livetv/cctv1} hostname{58.200.131.2} port{1935}
    av_url_split(proto, sizeof(proto), auth, sizeof(auth),
                 hostname, sizeof(hostname), &port,
                 path, sizeof(path), s->filename); //@wss add:从filename分解rtmp协议各部分字段

    n = strchr(path, ' '); //@wss add:检测到path中有空格
    if (n) {
        av_log(s, AV_LOG_WARNING,
        ...
        *n = '\0'; // Trim not supported part @wss add:空格字符前面加上\0字符结束符
    }

    if (auth[0]) { //@wss add:认证信息,RTMP携带认证信息？ 以@字符开始
        char *ptr = strchr(auth, ':');
        if (ptr) {
            *ptr = '\0';
            av_strlcpy(rt->username, auth, sizeof(rt->username)); //@wss add:copy认证信息中携带的用户名和密码
            av_strlcpy(rt->password, ptr + 1, sizeof(rt->password));
        }
    }

    if (rt->listen && strcmp(proto, "rtmp")) { //@wss add:proto不是rtmp
        av_log(s, AV_LOG_ERROR, "rtmp_listen not available for %s\n",
               proto);
        return AVERROR(EINVAL);
    }
    if (!strcmp(proto, "rtmpt") || !strcmp(proto, "rtmpts")) { //@wss add:rtmp或者rtmpts
        if (!strcmp(proto, "rtmpts")) //@wss add:如果是rtmpts，设置ffrtmphttp_tls为1
            av_dict_set(opts, "ffrtmphttp_tls", "1", 1);

        /* open the http tunneling connection */
        ff_url_join(buf, sizeof(buf), "ffrtmphttp", NULL, hostname, port, NULL); //@wss add:rtmp + http rtmp封装在http之上，可穿透防火墙 ffrtmphttp://58.200.131.2:1935
    } else if (!strcmp(proto, "rtmps")) {
        /* open the tls connection */
        if (port < 0)
            port = RTMPS_DEFAULT_PORT;
        ff_url_join(buf, sizeof(buf), "tls", NULL, hostname, port, NULL); //@wss add:rtmp + tls 类似rtmpt增加了TLS/SSL的安全功能 tls://58.200.131.2:1935
    } else if (!strcmp(proto, "rtmpe") || (!strcmp(proto, "rtmpte"))) {
        if (!strcmp(proto, "rtmpte"))
            av_dict_set(opts, "ffrtmpcrypt_tunneling", "1", 1);

        /* open the encrypted connection */
        ff_url_join(buf, sizeof(buf), "ffrtmpcrypt", NULL, hostname, port, NULL); //@wss add:rtmp + crypt rtmp的加密版本 ffrtmpcrypt://58.200.131.2:1935
        rt->encrypted = 1;
    } else {
        /* open the tcp connection */
        if (port < 0)
            port = RTMP_DEFAULT_PORT;
        if (rt->listen) //@wss add:用于区分是服务端和客户端，服务端listen标志置1
            ff_url_join(buf, sizeof(buf), "tcp", NULL, hostname, port,
                        "?listen&listen_timeout=%d&tcp_nodelay=%d",
                        rt->listen_timeout * 1000, rt->tcp_nodelay); //@wss add:buf中存储的是tcp://58.200.131.2:1935?listen&listen_timeout= &tcp_nodelay=
        else
            ff_url_join(buf, sizeof(buf), "tcp", NULL, hostname, port, "?tcp_nodelay=%d", rt->tcp_nodelay); //@wss add:rtmp + tcp rtmp的默认版本 tcp://58.200.131.2:1935?tcp_nodelay=1
    }

reconnect:
    if ((ret = ffurl_open_whitelist(&rt->stream, buf, AVIO_FLAG_READ_WRITE,
                                    &s->interrupt_callback, opts,
                                    s->protocol_whitelist, s->protocol_blacklist, s)) < 0) { //@wss add:这次执行和之前那次执行的url不同，协议不同(默认情况下打开tcp链接)
        av_log(s , AV_LOG_ERROR, "Cannot open connection %s\n", buf);
        goto fail;
    }

    if (rt->swfverify) { //@wss add:swf是flash动画
        if ((ret = rtmp_calc_swfhash(s)) < 0)
            goto fail;
    }

    rt->state = STATE_START;
    if (!rt->listen && (ret = rtmp_handshake(s, rt)) < 0) //@wss add:客户端握手
        goto fail;
    if (rt->listen && (ret = rtmp_server_handshake(s, rt)) < 0) //@wss add:服务端握手
        goto fail;

    rt->out_chunk_size = 128; //@wss add:RTMP chunck size
    rt->in_chunk_size  = 128; // Probably overwritten later
    rt->state = STATE_HANDSHAKED; //@wss add:转变状态为握手态
    ...
    if (!rt->playpath) {
        ...
        if (fname) {
            ...
            if (!strchr(fname, ':') && len >= 4 &&
                ...
                memcpy(rt->playpath, "mp4:", 5); //@wss add:如果是f4v或者mp4格式，统一转成mp4
            } else {
                if (len >= 4 && !strcmp(fname + len - 4, ".flv")) //@wss add:其他的则是flv格式
                    fname[len - 4] = '\0';
                rt->playpath[0] = 0;
            }
    ...
    if (!rt->tcurl) {
        ...
        ff_url_join(rt->tcurl, TCURL_MAX_LENGTH, proto, NULL, hostname,
                    port, "/%s", rt->app); //@wss add:设置tcurl flvurl
    }
    ...
    if (!rt->listen) {
        if ((ret = gen_connect(s, rt)) < 0) //@wss add:客户端链接
            goto fail;
    } else {
        if ((ret = read_connect(s, s->priv_data)) < 0) //@wss add:服务端链接
            goto fail;
    }
