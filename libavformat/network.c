...
#endif
    {
        fd = socket(af, type, proto); //@wss add:生成一个socket描述符 af:地址族规范(AF_INET2:IPV4 AF_INET6:IPV6...) type:套接字规范(SOCK_STREAM...) proto:协议(IPPROTO_TCP,IP_PROTO_UDP...)
#if HAVE_FCNTL
        if (fd != -1) {
              ...
              socklen_t addrlen, void *logctx)
{
    int ret;
    int reuse = 1;
    if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse))) { //@wss add:设置允许重用本地地址和端口
        av_log(logctx, AV_LOG_WARNING, "setsockopt(SO_REUSEADDR) failed\n");
    }
    ret = bind(fd, addr, addrlen); //@wss add:绑定地址
    if (ret)
        return ff_neterrno();

    ret = listen(fd, 1); //@wss add:监听端口
    if (ret)
        return ff_neterrno();
    return ret;
}
...
int ff_accept(int fd, int timeout, URLContext *h)
{
    ...
    ret = accept(fd, NULL, NULL); //@wss add:尝试链接 返回的是客户端包好的socket
    if (ret < 0)
        return ff_neterrno();
    if (ff_socket_nonblock(ret, 1) < 0) //@wss add:设置为非阻塞模式
                   ...
                   socklen_t addrlen, int timeout, URLContext *h)
{
    ...
    if ((ret = ff_accept(fd, timeout, h)) < 0)
        return ret;
    closesocket(fd); //@wss add:为什么把端口套接字关了
    return ret;
}
...
{
    struct addrinfo **next = &base->ai_next; //@wss add:主机名可能被映射成多个套接字结构，通过ai_next保存地址
    while (*next) {
        struct addrinfo *cur = *next;
        // Iterate forward until we find an entry of a different family.
        if (cur->ai_family == base->ai_family) { //@wss add:与基地址的地址族相同，如都是IPV4
            next = &cur->ai_next;
            continue;
        }
        if (cur == base->ai_next) { //@wss add:紧跟着第一个的地址族就不相同
            // If the first one following base is of a different family, just
            // move base forward one step and continue.
            base = cur; //@wss add:更新base和next指针指向当前地址
            next = &base->ai_next;
            continue;
        }
        ...
        *next = cur->ai_next;
        ...
        cur->ai_next = base->ai_next; //@wss add:保持全部地址族一致的地址？
        base->ai_next = cur;
                                 ...
                                 void (*customize_fd)(void *, int), void *customize_ctx)
{
    ...
    *ptr = ai->ai_next; //@wss add:更新ptr指针指向下一个套接字描述符

    attempt->fd = ff_socket(ai->ai_family, ai->ai_socktype, ai->ai_protocol, h); //@wss add:生成一个套接字描述符
    if (attempt->fd < 0)
        return ff_neterrno();
    attempt->deadline_us = av_gettime_relative() + timeout_ms * 1000;
    attempt->addr = ai;

    ff_socket_nonblock(attempt->fd, 1); //@wss add:设置socket为非阻塞模式

    if (customize_fd)
        customize_fd(customize_ctx, attempt->fd); //@wss add:设置套接字属性 接收/发送缓冲区大小，nodelay属性等

    while ((ret = connect(attempt->fd, ai->ai_addr, ai->ai_addrlen))) { //@wss add:链接
        ret = ff_neterrno();
        switch (ret) {
        case AVERROR(EINTR):
            if (ff_check_interrupt(&h->interrupt_callback)) {
                        ...
                        void (*customize_fd)(void *, int), void *customize_ctx)
{
    ...
    if (parallel > FF_ARRAY_ELEMS(attempts))
        parallel = FF_ARRAY_ELEMS(attempts); //@wss add:最多同时处理3个
    ...
    // an updated pointer.
    interleave_addrinfo(addrs); //@wss add:维持addrs中的套接字地址族一致
    ...
    while (nb_attempts > 0 || addrs) {
        ...
        if (nb_attempts < parallel && addrs) {
            getnameinfo(addrs->ai_addr, addrs->ai_addrlen,
                        hostbuf, sizeof(hostbuf), portbuf, sizeof(portbuf),
                        NI_NUMERICHOST | NI_NUMERICSERV); //@wss add:获取描述主机的字符串和描述服务的字符串
            ...
            last_err = start_connect_attempt(&attempts[nb_attempts], &addrs,
                                             ...
                                             customize_fd, customize_ctx); //@wss add:生成套接字，设置属性并进行connect 成功返回1
            if (last_err < 0) {
            ...
            if (last_err > 0) { //@wss add:如果connect成功了，之后的都不在尝试，把之前的socket close然后返回
                for (i = 0; i < nb_attempts; i++)
                    closesocket(attempts[i].fd);
                *fd = attempts[nb_attempts].fd;
                return 0;
            }
            pfd[nb_attempts].fd = attempts[nb_attempts].fd; //@wss add:last_err = 0 走到这 更新参数
            pfd[nb_attempts].events = POLLOUT;
            next_attempt_us = av_gettime_relative() + NEXT_ATTEMPT_DELAY_MS * 1000;
            nb_attempts++;
        }

        av_assert0(nb_attempts > 0); //@wss add:断言，有返回值为0的情况
        ...
        if (nb_attempts < parallel && addrs) //@wss add:有的connect失败了
        ...
        for (i = 0; i < nb_attempts; i++) {
            ...
            if (pfd[i].revents) {
                ...
                if (getsockopt(attempts[i].fd, SOL_SOCKET, SO_ERROR, &last_err, &optlen)) //@wss add:获取socket选项
