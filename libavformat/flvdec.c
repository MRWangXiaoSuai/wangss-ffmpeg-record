...
static int probe(const AVProbeData *p, int live)
{
    ...
    unsigned offset = AV_RB32(d + 5); //@wss add:获得第6字节-9字节数据，并做大小端转换(FLV以大端方式存储数据，操作系统以小端方式存储数据)
    ...
    if (d[0] == 'F' &&
        ...
        offset > 8) {
        int is_live = !memcmp(d + offset + 40, "NGINX RTMP", 10); //@wss add:对比flv协议头更易懂

        if (live == is_live)
            return AVPROBE_SCORE_MAX; //@wss add:匹配，获得最高分100
    }
    return 0;
}
