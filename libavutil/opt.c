...
{
    const AVClass *class;
    if (!obj) //@wss add:若obj对象为NULL，option肯定也不存在，返回NULL
        return NULL;
    //@wss add:obj是一个对象的指针，其指向的对象的实体的第一个参数是一个指向AVClass对象的指针
    class = *(const AVClass**)obj;
    if (!last && class && class->option && class->option[0].name) //@wss add:AVClass.option不为NULL，即指向有效的AVOption数组，迭代访问option选项
        return class->option; //@wss add:返回AVOption数组的第一个成员
    if (last && last[1].name) //@wss add:如果last不为NULL，即已被指定，last[1].name不为NULL，last指向的AVOption的下一个AVOption的name成员不为空
        return ++last; //@wss add:返回AVOtion数组的last的下一个AVOption，注意一般AVOption数组的最后一项是NULL
    return NULL;
}
...
{
    const AVOption *opt = NULL;
    while ((opt = av_opt_next(s, opt))) { //@wss add:迭代获取option值
        void *dst = ((uint8_t*)s) + opt->offset; //@wss add:s->av_class->option与s的成员是对应关系 AVOption的offset属性定义了s对象对应的成员相对于s对象首地址的偏移offset

        if ((opt->flags & mask) != flags) //@wss add:mask和flag用于控制需要修改设置哪些参数
            continue;

        if (opt->flags & AV_OPT_FLAG_READONLY) //@wss add:如果属性是只读，那么不设置
            continue;
    ...
    if (search_flags & AV_OPT_SEARCH_CHILDREN) {
        if (search_flags & AV_OPT_SEARCH_FAKE_OBJ) {
            ...
            while (child = av_opt_child_class_iterate(c, &iter))
                if (o = av_opt_find2(&child, name, unit, opt_flags, search_flags, NULL)) //@wss add:迭代找子树，找到最底层节点
                    return o;
        } else {
            void *child = NULL;
...
int av_opt_copy(void *dst, const void *src)
{
    ...
    c = *(AVClass **)src;
    if (!c || c != *(AVClass **)dst) //@wss add:AVClass是链接option和context的桥梁 所有的option信息都在AVClass里面
        return AVERROR(EINVAL);
    //@wss add:循环获取src选项，然后将选项对应的src成员值copy到dst成员中
    while ((o = av_opt_next(src, o))) {
        void *field_dst = (uint8_t *)dst + o->offset; //@wss add:指向目标成员的指针
        void *field_src = (uint8_t *)src + o->offset; //@wss add:指向原始成员的指针
        uint8_t **field_dst8 = (uint8_t **)field_dst; //@wss add:指向目标成员的指针(注意参数类型不同)
        uint8_t **field_src8 = (uint8_t **)field_src; //@wss add:指向原始成员的指针(注意参数类型不同)
        //@wss add:根据option类型 copy 字符串类型
        if (o->type == AV_OPT_TYPE_STRING) {
            if (*field_dst8 != *field_src8) //@wss add:判断源与目的串是否是同一个，若不是，释放目的串空间，是的话就不能释放，否则源串也没了，源串指针变成了悬空指针
                av_freep(field_dst8);
            *field_dst8 = av_strdup(*field_src8); //@wss add:分配空间并copy源串到目的串
            if (*field_src8 && !*field_dst8)
                ret = AVERROR(ENOMEM);
        } else if (o->type == AV_OPT_TYPE_BINARY) { //@wss add:二进制类型
            int len = *(int *)(field_src8 + 1); //@wss add:求取二进制块大小
            if (*field_dst8 != *field_src8) //@wss add:判断源二进制块与目的二进制块是否是同一块，不是则释放
                av_freep(field_dst8);
            *field_dst8 = av_memdup(*field_src8, len); //@wss add:分配空间并copy源二进制块到目的二进制块
            if (len && !*field_dst8) { //@wss add:判断目的二进制块是否分配并copy成功不成功要把len置0，保证二者一致性
                ret = AVERROR(ENOMEM);
                len = 0;
            }
            *(int *)(field_dst8 + 1) = len; //@wss add:最后设置二进制块大小
        } else if (o->type == AV_OPT_TYPE_CONST) { //@wss add:const常量类型
            // do nothing
        } else if (o->type == AV_OPT_TYPE_DICT) { //@wss add:字典类型
            AVDictionary **sdict = (AVDictionary **) field_src; //@wss add:转换地址类型
            AVDictionary **ddict = (AVDictionary **) field_dst;
            int ret2;
            if (*sdict != *ddict) //@wss add:判断源AVDictionary与目的AVDictionary是否是同一个，不是则释放
                av_dict_free(ddict);
            *ddict = NULL;
            ret2 = av_dict_copy(ddict, *sdict, 0); //@wss add:分配空间并copy源AVDictionary到目的AVDictionary
            if (ret2 < 0)
                ret = ret2;
