...
int avpriv_open(const char *filename, int flags, ...)
{
    int fd;
    unsigned int mode = 0;
    va_list ap; //@wss add:ap是指向参数的指针

    va_start(ap, flags); //@wss add:初始化刚定义的变量，flags是最后一个显式声明的参数，用以获得第一个变长参数的位置
    if (flags & O_CREAT)
        mode = va_arg(ap, unsigned int); //@wss add:返回变长参数的值，第二个参数是变长参数的类型
    va_end(ap); //@wss add:将ap重置为NULL
