...
//@wss add；这里参数其实是个二级指针，为什么没表示成void**? 因为void**不是任意指针类型都可以为其赋值，void*可以
void av_freep(void *arg)
{
    void *val;

    memcpy(&val, arg, sizeof(val)); //@wss add:这里copy的其实是指针，现在指针由val管理
    memcpy(arg, &(void *){ NULL }, sizeof(val)); //@wss add:指针置NULL，解决了av_free带来的悬空指针问题
    av_free(val); //@wss add:释放指针指向的内存
}
...
char *av_strdup(const char *s)
{
    char *ptr = NULL;
    if (s) {
        size_t len = strlen(s) + 1; //@wss add:计算长度，C字符串最后一位是'\0'，因此长度需要+1
        ptr = av_realloc(NULL, len); //@wss add:av_realloc是在原地址空间上扩展，av_malloc是重新分配一块内存
        if (ptr)
            memcpy(ptr, s, len);
    }
    return ptr;
