...
int av_match_name(const char *name, const char *names)
{
    ...
    namelen = strlen(name);
    while (*names) { //@wss add:如果包含多个格式，则对name再匹配一次
        int negate = '-' == *names;
        p = strchr(names, ','); //@wss add:有些格式对应多种名称，如MKV格式的解复用器，.name="matroska,webm"
        if (!p) //@wss add:就一种格式
            p = names + strlen(names);
        names += negate;
        len = FFMAX(p - names, namelen);
        if (!av_strncasecmp(name, names, len) || !strncmp("ALL", names, FFMAX(3, p - names))) //@wss add:ALL代表可能为任何格式(实际保存文件时可选择.all格式)
            return !negate;
        names = p + (*p == ',');
    }
    return 0;
