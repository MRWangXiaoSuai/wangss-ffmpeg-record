...
{
    WorkerContext *w = v;
    AVSliceThread *ctx = w->ctx; //@wss add:slice级别的线程上下文参数

    pthread_mutex_lock(&w->mutex);
    pthread_cond_signal(&w->cond);

    while (1) {
        w->done = 1; //@wss add:初始化为0，线程启动置为1，似乎是在等待什么条件发生
        while (w->done)
        ...
        if (run_jobs(ctx)) { //@wss add:真正的job执行函数
                              ...
                              int nb_threads)
{
    ...
    if (!nb_threads) {
        ...
        if (nb_cpus > 1)
            nb_threads = FFMIN(nb_cpus + 1, MAX_AUTO_THREADS); //@wss add:再更新一次线程数量
        else
            nb_threads = 1;
    }
    ...
    *pctx = ctx = av_mallocz(sizeof(*ctx)); //@wss add:为slice线程分配内存，此处，*Pctx与ctx指向同一块地址+
    if (!ctx)
        return AVERROR(ENOMEM);

    if (nb_workers && !(ctx->workers = av_calloc(nb_workers, sizeof(*ctx->workers)))) { //@wss add:分配内存
        av_freep(pctx);
        return AVERROR(ENOMEM);
    }
    ...
    ctx->nb_jobs     = 0;
    ctx->finished    = 0; //@wss add:这里所有参数同时由pctx带出去
    ...
    for (i = 0; i < nb_workers; i++) {
        ...
        if (ret = pthread_create(&w->thread, NULL, thread_worker, w)) { //@wss add:创建线程，传递参数w，w的ctx带着参数
