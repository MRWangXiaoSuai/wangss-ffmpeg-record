                ...
                int log_offset, void *log_ctx)
{
    ...
    int err, fd = avpriv_open(filename, O_RDONLY); //@wss add:只读方式打开文件
    struct stat st;
    av_unused void *ptr;
    off_t off_size;
    char errbuf[128];
    ...
    *size = 0;
    ...
    if (fstat(fd, &st) < 0) { //@wss add:_fstat64i32函数获取文件信息
        err = AVERROR(errno);
        av_strerror(err, errbuf, sizeof(errbuf));
    ...
    off_size = st.st_size;//@wss add:文件大小
...
#if HAVE_MMAP
    ptr = mmap(NULL, *size, PROT_READ|PROT_WRITE, MAP_PRIVATE, fd, 0); //@wss add:mmap将一个文件或者其它对象映射进内存。让用户程序直接访问设备内存,效率更高
    if (ptr == MAP_FAILED) {
        err = AVERROR(errno);
        av_strerror(err, errbuf, sizeof(errbuf));
...
#elif HAVE_MAPVIEWOFFILE
    {
        HANDLE mh, fh = (HANDLE)_get_osfhandle(fd); //@wss add:检索与指定的文件描述符关联的操作系统文件句柄

        mh = CreateFileMapping(fh, NULL, PAGE_READONLY, 0, 0, NULL); //@wss add:创建一个文件映射内核对象
        if (!mh) {
        ...
        ptr = MapViewOfFile(mh, FILE_MAP_READ, 0, 0, *size); //@wss add:将内存映射文件映射到进程的虚拟地址中
        CloseHandle(mh);
        if (!ptr) {
    ...
    *bufptr = av_malloc(*size);
    ...
    read(fd, *bufptr, *size); //@wss add:传统copy方式
#endif
